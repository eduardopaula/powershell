...
$extensionFilepath = Read-Host 'Specify filepath for extension (.zip file)'
...
# Get coordinator hostnames
$coordinatorHosts = @()
do {
    $input = Read-Host "Enter a coordinator hostname or IPv4 address (or press enter to finish)"
    if ($input -ne '') { $coordinatorHosts += $input }
} until ($input -eq '')

# Generate extension
if($coordinatorHosts.count -eq 0) {
	New-CAQRadarExtension -Connection $connection -ExtensionFilepath $extensionFilepath -subscriptionId $subscriptionId
}
else {
	New-CAQRadarExtension -Connection $connection -ExtensionFilepath $extensionFilepath -subscriptionId $subscriptionId -CoordinatorHosts $coordinatorHosts
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAjllsSDHmh6ycJ
# 9EU3aj2DoxttxqSL11fMO65hJTpruqCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCDN67XulF0G/zo4FGtnjRuXHtu/MNAOsteCt0nFG6DsVDANBgkqhkiG
# 9w0BAQEFAASCAQBpmOxwRrR7LsPwWkiC+FWyXMqTSZptqt7dpyL4ibTs1crE2eKu
# /UgGVOmugyp3SCl8LbPx3i4I4mahUh+3jTNFMPbnS3ZPLCrBMTgdEKiNtL6hoYsk
# FZk5e+Qj8cnNX3Pcuu9M/AuvKW+V7VDNSXqc/XHaL0wqTGPEzQ/QCQsSaAZzBFL/
# t+Zy1uiVVMjptElwB/1BrKuN6E8Fm2zr8HqiFlMsUxtnvoY9KL8u3m/ZQS1nBkcH
# z20lxvtzX1JnC0TnJhJc6IRMH1Sgr6a5bE0Ll9XvbR6Hck9H1K9OmIQduYs06O9A
# HO+RmghxxI2OoHK5MuXDlh97FWeLvrLw8A76oYIXQDCCFzwGCisGAQQBgjcDAwEx
...
# BQAEINxiPqCgDfQ3kdHF6oCblaPaepa9Ts70mspIHF4BSFmxAhEAlUQqJRbudZeP
# l0dcDDE2uhgPMjAyMzExMTMxNTMzNTdaoIITCTCCBsIwggSqoAMCAQICEAVEr/OU
...
# DQEJBTEPFw0yMzExMTMxNTMzNTdaMCsGCyqGSIb3DQEJEAIMMRwwGjAYMBYEFGbw
# KzLCwskPgl3OqorJxk8ZnM9AMC8GCSqGSIb3DQEJBDEiBCCyzzfPk8Qw0C4fERWN
# KyHZpH1CEIIRxbdVmy+CVtcTnzA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDS9uRt
# 7XQizNHUQFdoQTZvgoraVZquMxavTRqa1Ax4KDANBgkqhkiG9w0BAQEFAASCAgB0
# QkTmJcJR3gxeUWZbyWLcItqqWMohUh9uKFUl4pnX7IsdsGNzJCSv7jienfAn9obK
# XeZHBumtIvp3XsQLLXCyR7sHiKYcZz/HwxNWB/saiV1afN0CSPllCHoJZ9xspej0
# ruIjVdNqyRWYT3zCO0FXpeubDlzL4uBDG+8h0z6SgnEwzD9yl/wYHoRzAxS1k4/u
# UkDI4dCXu8RcrR3/H4fZcDJiKL/RIWgJmde7SfxsUfRgRbPoNAb/Rgfy5v7Z5gKa
# YbSGjaPSULKlre1AWpbzaMWZma9dtu9ans6f2iy4sWgonvqqdhUmS0Sq3D4zF+Ej
# 3Pg+nKJ7mtBKp9oCnKv4E4I3qcU6eeL8agB9C4j3gQeT/M2id3PkuCbTwMk6LuFR
# A5AgWGdcMWj/Muq+n0oIYho5bCH9XAHNCsQ/AEOD3oxO7eVFMtDYdyeIl5xUCjui
# sQZu22O1FpC8lonQ0WzRLCKBK5BiFUmmlpNOl0LiCLkdtU/YJy5tvcpV+swZnD0C
# cf6DTnpD0csfCJzx3V6RXm7ZtWAQ0sGImqaOG6Vmv+7dHhwrCwKYWn6LXB2zPOfG
# Nw6gVCLNrL28knlf94wuuR3ovDoB83irMMRr8VPWAjN033U34UctwVTzqU9xChNx
# DCxsKw7HRwukdpxj5n5DHjauNd/2ajeESoLp8U+scQ==

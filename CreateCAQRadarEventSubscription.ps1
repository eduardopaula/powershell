...
# Import Change Audtor PowerShell module
...
# Connect Change Auditor PowerShell Client to installation
$connection = Connect-CAClient -InstallationName $installationName

# Enable/disable TLS
$tlsEnabled = Read-Host 'Specify whether to use TLS encrypted event forwarding (true or false)'
if ($tlsEnabled -eq "false"){
	$tlsEnabled = $false
}
else{
	$tlsEnabled = $true
...
# Get host and port
$qradarHost = Read-Host 'Specify the QRadar server hostname'
$qradarPort = Read-Host 'Specify the QRadar server port  (or skip for default)'
if ([string]::IsNullOrEmpty($qradarPort)){
	if ($tlsEnabled){
		$qradarPort = 6514
	}
	else{
...
$allowedCoordinators = @()
do {
    $input = Read-Host "Enter an allowed coordinator DNS or NetBIOS name (or press enter to finish)"
    if (![string]::IsNullOrEmpty($input)) { $allowedCoordinators += $input }
} until ([string]::IsNullOrEmpty($input))

# Create subscription
if ($allowedCoordinators.Length -eq 0){
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA80qT7CY8H8CrW
# +pakLeSU6LElZdVP8PL8fKKPXj4P7KCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCCXkofjtJVInz0pOLQ/pizmm0lpKksQlApSS8RSBUUoszANBgkqhkiG
# 9w0BAQEFAASCAQCKNmPsf7hE3O99eQTmJwPM01Lk6k7OyyNO7vxs283lUo09M90L
# 9ucWHEOI+HAFy5jo3X5CY8EXoSYu3QQ3RduJyVHUBEMrSRfbs4qizbbPKt76Yn33
# 7D8agkUyLxOPGzQeW1Kb4f6n38ChTqLt6ZyH08C1wPl++k0Tg4/YOE7j/RvI9g4A
# vGS02wF+ZdP7KLFf7nzmDS15E7yDXGjEMVkTc3EaAF67YXb3sJPPtsfnn/EjJHnM
# LCoFgou5UE1RPWzH+ufrcafI3WzfSPcqhsjPwt4OBndRMtPE6H/aGesT8ZLEtU0/
# Hjgp7clyQeTYzCnubtqhOkcOoX5uG1gIbVj3oYIXPzCCFzsGCisGAQQBgjcDAwEx
...
# BQAEIOerioONAz5IbQizB7fR4FnXZVoiBUHFdvipkcTYNbxkAhA4Tc1fQA90s8Zq
# yzdOPOdTGA8yMDIzMTExMzE1MzM1N1qgghMJMIIGwjCCBKqgAwIBAgIQBUSv85Sd
...
# AQkFMQ8XDTIzMTExMzE1MzM1N1owKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQUZvAr
# MsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEIBqvdOvzik2zD9kpDGlu
# pvGX1c0oHQvk/fhp2a95ZXDpMDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3t
# dCLM0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICAIH5
# ltV9fj+FpEZLldseILePmSt7RpKw+qVEfiJ3tz4b0dPIZeD40iqw0Pyw1muSTC+/
# 5rvA27zi/tLccaFh7amTmQ7tTghYp7ySOcwP1QbwB0OHX/08SCrc2UIwqDsz04rr
# +rERG5c5+KYM/GbvkP5dFTdiy8TLFsZVmPKgjjC+UUjv4n9cQ4ifIAgfZEz8A/UY
# v34JRCXXCTIfTBy6sRpqtltXU/ZqQVpjIwyuBduSShvRdKxSxYT0glfymiZd+kFL
# Ok4SajeW99fs7uh8HG8pbmnGYI2yRGVrQKQI25C28ftqDTK3WBPF/781o0RQlWzj
# JSoOxB3F3X8pZBD4Omp7Ee7xt/wr4NkJN4l+Kn9icywhyjoA/FsWfS9KDfoQO6lR
# PPHwCu9yqH3X0FfSmjmrrsqMi7aVoeRFQXoLHm28xeOVAmxBdw+M8JlYl3rOYJX2
# Q34Ew5lOSfRhvZXBV9/AjTluM9yFyXsXAmBDfiTFVbXJbmNctRy2Hy+NVROa+IKB
# mY6o61UMAupN+xl5d68e6yhczPJ0sdRIN2IqQRwcLTkCmQ5fpgqB/yK8pWGUgR49
# 3N1IL9Nxfrl0htwU8i29uAUkr+Y0RtaXaiatQf/fRi7Id7QVprr0zUh8N+T3pas6
# kZxJT5yoKQgDt0TCgiTi3cguGWpJZ6WBZe+PNMKI

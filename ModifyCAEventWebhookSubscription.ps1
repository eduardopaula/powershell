...
function BooleanPrompt( $message, [REF] $output )
{
    $hint = $message + ' ("True", "False" or "" for no change)'
    $textInput = Read-Host $hint
    if ($textInput -eq "")
    {
        $output.Value = $null
    }
    ...
    {
        $output.Value = [System.Convert]::ToBoolean($textInput)
    }
}

function IntegerPrompt( $message, [REF] $output )
{
    $hint = $message + ' ("" for no change)'
    $textInput = Read-Host $hint
    if ($textInput -eq "")
    {
        $output.Value = $null
    ...
    {
        $output.Value = [System.Convert]::ToInt32($textInput)
    }
}

function StringPrompt( $message, [REF] $output )
{
    $hint = $message + ' ("" for no change)'
    $textInput = Read-Host $hint
    if ($textInput -eq "")
...
$subscriptions = Get-CAEventWebhookSubscriptions -Connection $connection | ForEach-Object -Process { New-Object PSObject -Property @{Id=$_.Id; Url=$_.NotificationUrl; Webhook=$_ }}

if ($subscriptions -is [array])
{
    Write-Host 'Subscriptions: '
    $global:index = 0
    $subscriptions | Format-Table -Property @{name="#";expression={$global:index;$global:index+=1}},Id,Url -AutoSize

    $toModify = Read-Host 'Specify subscription # to modify'

    if (($toModify -lt 1) -or ($toModify -gt $subscriptions.Length))
    {
        throw [System.ArgumentException] "$toModify is not a valid subscription number."
    }

    $subscription = $subscriptions[$toModify - 1].Webhook
}
else
{
    $subscription = $subscriptions.Webhook
}

Write-Host ($subscription | Format-List | Out-String)

[Nullable[bool]]$newEnabled = $null
BooleanPrompt 'Enter a new value for Enabled' ([REF] $newEnabled)

[Nullable[int]]$newNotificationInterval = $null
IntegerPrompt 'Enter a new value for NotificationInterval' ([REF] $newNotificationInterval)

[Nullable[int]]$newHeartbeatInterval = $null
IntegerPrompt 'Enter a new value for HeartbeatInterval' ([REF] $newHeartbeatInterval)

[Nullable[int]]$newBatchSize = $null
IntegerPrompt 'Enter a new value for BatchSize' ([REF] $newBatchSize)

[string]$newAuthId = $null
StringPrompt 'Enter a new value for AuthorizationId' ([REF] $newAuthId)

[string]$newUrl = $null
StringPrompt 'Enter a new value for NotificationUrl' ([REF] $newUrl)

[string]$heartbeatUrl = $null
StringPrompt 'Enter a new value for HeartbeatUrl' ([REF] $heartbeatUrl)

[string[]]$allowedCoordinators = $null
$allowedCoordinators = (Read-Host 'Enter a new value for Allowed Coordinators (ALL for all coordinators, "" for no change, separate names with commas)').split(",") | %{$_.trim()}
if ($allowedCoordinators.Count -eq 1)
{
    if ( $allowedCoordinators[0].Equals("all", [System.StringComparison]::OrdinalIgnoreCase))
    {
        ### empty array is the default for new subscriptions, means "all coordinators"
        $allowedCoordinators = @()
    }
    else 
    {
        if ( $allowedCoordinators[0].Length -eq 0)
        {
            ### Read-Host return with no data entry returns one zero-length string, means "no change"
            $allowedCoordinators = $null
        }
    }
}

$updatedSubscription = Set-CAEventWebhookSubscription -Connection $connection -SubscriptionId $subscription.Id -Enabled $newEnabled -NotificationInterval $newNotificationInterval -HeartbeatInterval $newHeartbeatInterval -BatchSize $newBatchSize -AuthorizationId $newAuthId -NotificationUrl $newUrl -HeartbeatUrl $heartbeatUrl -AllowedCoordinators $allowedCoordinators

Write-Host ($updatedSubscription | Format-List | Out-String)
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBRyWmUII/+OIzD
# MmD0ZPmY86phzzP7pG6K2TSgFMUwqKCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCCHLjH/3tCDrmXH3vd8Y7Q15P/nWDnssD1GKsChHuMWeDANBgkqhkiG
# 9w0BAQEFAASCAQBfKc+z5pSqM32h+j+Yg7E0iuAs6UJlZVdLYGzKYDDX7tNEA1KV
# d/3FnLy9lARWSsa4HIq6f4PSUR6VMQkrch5DwSTetrO42o6DZQrePW9PSndyZgt8
# WfY52w/jKjat5U8tvFe6yHavHUpaAqWksYVNp2JIu+MpoWCLz4peLuQHGTZkAsaq
# 5h7Px+001wLj3jco8KJesoGP/mxbdDWdKOSQmjP2uPbswkZ7XZBVfLnxUvYVInnT
# cd7OAldHBoMZFsWoTOQCiy0jlh7J36Hn/Mqc6cLTexjryFAyREcSJzHjO3uTIda6
# MYL+s3NaSAhcJ7FbYm5hAu4nvGTAh3477laOoYIXPzCCFzsGCisGAQQBgjcDAwEx
...
# BQAEIBVCt9owr4k+ylSQ7J6u+CZLfsoCy9pnoRpqjNCVjE81AhA3lIHVLJgnOhyc
# x5iaPVAHGA8yMDIzMTExMzE1MzUzOFqgghMJMIIGwjCCBKqgAwIBAgIQBUSv85Sd
...
# AQkFMQ8XDTIzMTExMzE1MzUzOFowKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQUZvAr
# MsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEIFasSns9JEPBtOjQfqcQ
# aicTSDTjeHHltgRIcMDkFeVFMDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3t
# dCLM0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICAE+C
# uSK0GXO4xP28BVhAMJxfCGpxFPbdvoCI8usVewYdfrkkz1ClNTC7mFV+1jNB0Pmc
# V6xZLfbD9NtERwQOxCCyAibbrSvdgevALZ1teF/YKw2vSEL34WjZKQ5APwmn6QN7
# HyAD1WE9JzROuuL2KjdJ6e0IYnO9m1qObKa/LFbDLlZtKfCCCyjOEUvrSKO+thMP
# i4Bdi2TBbJ2Ey7pyNcOHBUOgXbdG3KsCpCGByTuAc69xn2sHs4Dq9pl1SeaiJHyp
# 2ZDt4fwYT3zpChsucaIdVPqrwtfO8BX92B73swQE1TV4x9ccY5JzMEUT1qOh9aSx
# sB/kYMnsaQCw08QErCQ0NUBst+C+yq6Swo1MkDUT8xWae7TIgql6nJqf79rZi8pL
# CFcR+eakxN7adX67+7NdmIR86Q6CFlZmU8ju5ocpU3EqRY/ZWPZ8mzWZlt82GwZr
# ZpdMdReTf5ru7ud+OKmnN40Ns1fx2M/eb5bsSySlldHP5PFVXrcCTzSw5pn5Zm+h
# Fi+ohLtW018NE5VChfqgaOcGyyWxpUKIzCYa1iizInWe6uDRf4QwIG9517LPrMgP
# In5mrO/Tf52+3TnLsL5PcmlyetKysjQDKtHINzeSAyAdLmh6nbIMm7mBIU3sy7yT
# Mlo/xtLgU2fk3w4xmLKquPtp2oYa/fMsoYsRjD4G

...
$subscriptionId = Read-Host 'Specify the Subscription ID of an existing Splunk Event Subscription'
...
do {
    ...
    "     4. Heartbeat Token`n" +
    "     5. Enabled state`n" +
    "     6. Notification Interval`n" +
    "     7. Heartbeat Interval`n" +
    "     8. Batch Size`n" +
...
switch ($input) {
    '1' {
        # Modify the Splunk URL
        $newValue = Read-Host 'Specify a new Splunk URL'
    ...
    '2' {
        # Modify the Event Token
        $newValue = Read-Host 'Specify a new Event Token'
    ...
    '4' {
        # Modify the Heartbeat Token
        $newValue = Read-Host 'Specify a new Heartbeat Token'
        Set-CASplunkEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId `
            -HeartbeatToken $newValue
    }
    '5' {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBFNlOJ847bQ76k
# cwScQcv8l/Zd5Q67L1rlUeyCysnK/aCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCDqdO5/WBVQnvewDEBwupkmpvw9VeozO3UhkCYX04x+NjANBgkqhkiG
# 9w0BAQEFAASCAQBucPY/dYFB22RLDQnd7fiDztGzuYN7HSzypGx6TMYp/ENAZ+CL
# CxrRK8mQnoiP1LvnTNQIDxsNaXYZ4D/viXoNFK3w4dett/8kCp4RveMvKhRPclcu
# qcM8mXWPF4DWT7MRDN9eijAJRJsUJjHQirMTG3jZYY08/8rvh50pwvvUM/Hc87u6
# DfZsTn5Gc4nrfGxIXLxW4CxH/WLUmNumRMyqRN48hGay6kpBE2H3uKxR3pVUaVA2
# T1zM8lmHTIa5Hi4jZHAetjLW6XmT04mmNKxspimJd+ggey1T/ZHBM7wNV0MVg/5E
# GDnN32vXyfpZvK9H/gC3YdArWmR+to9u7UR2oYIXPzCCFzsGCisGAQQBgjcDAwEx
...
# BQAEIMCiSSrpQSQ/RaWx/ozMXxpnur9R2Ucg6MKf9e3OhkMVAhAM4ZMVpyVSZyB8
# p50+irmAGA8yMDIzMTExMzE1MzU0MFqgghMJMIIGwjCCBKqgAwIBAgIQBUSv85Sd
...
# AQkFMQ8XDTIzMTExMzE1MzU0MFowKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQUZvAr
# MsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEIB2iZ53bcQlIJMe5fF7t
# yIg0HwKFX0qnz03HDYnpZdaLMDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3t
# dCLM0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICAFKw
# fXJJqpKMPVKznQc4mIcF0xfxgvfN/A8WqKr6tWRrzIXVrJpVBeOuo0oV6S8s4pZQ
# UtPaucgL6SIbS64yvhupKAOewRiHmwMyKeltcVm89VtF9ekxi9crb13QZ/oGjFKw
# ct4Lb46laBnDr6IlP5vjPuJLEGrMrj+y2M/+e4hiW93pGx2UzEAZgFCIKsWRpyC1
# CwGjWFPkgp9ffYfEI9rL1WSMk5gKNem15UlLJyQIHSeciWWULUOaod/wORwBzST9
# MoWwE6hWg/6H0ZuN8QiKqWZmivGPocgmSXrbYUUo9p7W2TjjSvoyMGo/0uzEgqDW
# e4b3bkoqWvW5vqp1pXskpN7crMZq46hCCuhNNjHlzjdL8ynCKtc9S/LFGp4lfdov
# 8xZU5dubjy77GVOGRYUMuRYanHjgTcYAjDqjCS5YU8yTvrvfo1PU7WT4jq3qYAlu
# rS1KlNYzDN/WNynjvsI02ij1L1OYtK15CiTJ92z2N27n7+ZqF737hby8yaZHx5Ov
# 5fdV0YXw8N+GPbdXN3y/ChYmpLTvJUaEFMBVZPhNFJsx32C+d063EeujPAZismPI
# CCcisXJ5/FVsdptDVYJI3QJFoj50CeKhGJybh6suPA5XT6zxmlKzimMTDPFV7QTy
# K13HxxNtOYZPfBgF5FaPETwILRoWD3gE09R8zVQ1

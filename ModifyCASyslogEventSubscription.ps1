...
# Import the Change Audtor PowerShell module
...
$subscriptionId = Read-Host 'Specify the Subscription ID of an existing Syslog Event Subscription'
...
do {
    ...
    "     3. Heartbeat URL`n" +
    "     4. Enabled state`n" +
    "     5. Notification Interval`n" +
    "     6. Heartbeat Interval`n" +
    "     7. Batch Size`n" +
	"     8. Syslog Format`n" +
    "     9. Allowed Coordinators`n")
...
switch ($input) {
    '1' {
        # Modify the Syslog Host
        $newValue = Read-Host 'Specify a new Syslog Host'
    ...
    '2' {
        # Modify the Syslog Port
        $newValue = Read-Host 'Specify a new Syslog Port'
        Set-CASyslogEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId `
            -SyslogPort $newValue
    ...
    '3' {
        # Modify the Heartbeat URL
        $newValue = Read-Host 'Specify a new Heartbeat URL'
        Set-CASyslogEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId `
            -HeartbeatUrl $newValue
    }
    '4' {
    ...
    '5' {
        ...
        $newValue = Read-Host 'Specify a new Notification Interval'
        Set-CASyslogEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId `
    ...
    '6' {
        ...
        $newValue = Read-Host 'Specify a new Heartbeat Interval'
        Set-CASyslogEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId `
    ...
    '7' {
        ...
        $newValue = Read-Host 'Specify a new Batch Size'
        Set-CASyslogEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId `
            -BatchSize $newValue
	...
	'8' {
        # Modify the Syslog Format
        $newValue = Read-Host 'Specify a new Syslog Format (LEEF or CEF)'
        Set-CASyslogEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId `
            -SyslogFormat $newValue
    }
    '9' {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDSOeI4py+TnSrT
# t8y0RZMM2Knsn/tSe8Id8QXQA0Bg+aCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCBi5wQBS7wmijxaDu9/f9gAbVN+s+adQMCwWcF6e/ca8zANBgkqhkiG
# 9w0BAQEFAASCAQAzi5pB2y/f5nOCJ34PrDnkkLZXpfs5gNIF4gP5dTRnXNP1udZZ
# KDuzenHBy95/UlW4P/+o1ERtybkkMWNS2NXyTPpyy+xLTjxyoQovrb0pGDZOCrBh
# 8g1gIK2PKbVIyC9flSbozpu3j44v01gafDjA0LAkg58DO1l34CqH5Qht/lZb0Bg6
# tVNDNpv3Ou6jzdHBXDH3PX4UzADRmZfx7qarwlSBE5NdxJhOU9xQoPdiZL++dVVw
# UJiH2ph9bq6h82CQvwpFOfO//Nt3oBu5+vrqmUC9HUgqKMRJrEeTN1x0zNnPblPS
# YCD0z/6+ZNe21GOufAUymWHlVH/pPEALXWqQoYIXQDCCFzwGCisGAQQBgjcDAwEx
...
# BQAEIK7x4raTRfFE7HU9Swi6MBwFtIhIZ47jNBDdmnwxpj2rAhEAkFvgSNmn/HNs
# LZ++qtx7BhgPMjAyMzExMTMxNTM1NDBaoIITCTCCBsIwggSqoAMCAQICEAVEr/OU
...
# KzLCwskPgl3OqorJxk8ZnM9AMC8GCSqGSIb3DQEJBDEiBCCznfzs1e02C5k8aNAk
# L2Bagx7s/ZrYvLhYb4RIVMZbujA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDS9uRt
# 7XQizNHUQFdoQTZvgoraVZquMxavTRqa1Ax4KDANBgkqhkiG9w0BAQEFAASCAgBH
# JpljMmB7H4f5+o0YBXsenwx8E1NlMsWRzutkNZA+v4jMmJo0dHGAWprxtAMzo6+h
# Mkd5viyEdb3H0cHXwP17+IcAYwX3xjUnFOSv2aOL9qpfmrc7heNLXEPY4MjFq61A
# he8tOOjI/NNL+AequfMN/HOAMxk/3P6/IeGGERgAo0UlLZEvk9GezFdk4SNbtPa/
# 8Z/zukrWRP+xbTMibwJ0Gv8eenzY0SQkr7eaIYoTYwmRbxOZ8plsa25mnsa5oYtn
# NO29LWNaXbcpQ2hm1hZpNuBmAL6czONyWRWKDIE6qz1hG9ck6bHy4tLi66GJmDq9
# g80tsF2KIXShyzzgyQewndAgCXkR1h8VtPGWpAW6/FN8P1XAVqgHZoeWtRBPY1oF
# 9zFs5uF8Kk9mHn2/UeDpAISPhTpT891GtrclcLQkUlPm1zSSrOvCq1kK9PxI1VBj
# Eo9TW2bsycvmjg5Srxn8VDIF+U1brWPxp12lEkB6pNkE7r++PFNBrxC5GJLSRQrT
# 4KxOBrk/C4iz4UOME4Bq9qeC5VAeYMdHn8HdZiU+A2osEFi/rlENUKv5LmUNAksX
# Ye3ZJZkVSziZnPVw+OuwvGjU/AdWcgTDjzvsyI1jCSfWNnm/ouBwaeKRL05efrbs
# Bsu+9mJZz23hvUtEZnLPtv0F6P6nBc4hJklWL97WDA==

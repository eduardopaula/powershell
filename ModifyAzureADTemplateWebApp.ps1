...
# Get the Azure AD templates in the Change Auditor installation
$azureADtemplates = Get-CAAzureADTemplates -Connection $connection

# Prompt user for template name to modify
# Run Get-CAAzureADTemplates cmdlet to get a display name of the Azure AD template.
# Use the display name for the template name.
$templateName = Read-Host -Prompt 'Enter the name of the Azure AD template you want to modify'

# Search for specified template to modify
$selectedTemplate = $azureADtemplates | Where-Object {$_.DisplayName -eq $templateName}
...
# Use NEW to automatically create a replacement web app. Use EXISTING to update the secret key of an existing web app or provide a new user-created replacement web app. 
$updateType = Read-Host 'Enter "NEW" to create a new Web App, "EXISTING" to enter the App ID and App Key of an existing Web App [New],, "' 
...
{
    # Create the replacement web app and update the template. A browser window will be spawned to accept Azure logon credentials.
    Set-CAAzureADTemplate -Connection $connection  -Template $selectedTemplate -CreateWebApp 
}
else
{
    Write-Output "`n"
    Write-Output 'To use an existing Azure Active Directory web app ensure that the app has a '`
                 'valid, non-expired Client Secret and the permissions required by Change Auditor. '`
                 'For details see the Change Auditor "Office 365 and Azure Active Directory Auditing 7.0 User Guide"  '`
                 'section "Azure Active Directory auditing page" note "If you are using an existing web application".'
    Write-Output "`n"
    ...
    Set-CAAzureADTemplate -Connection $connection  -Template $selectedTemplate -WebAppId $webAppId -WebAppKey $appKey
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBg0BSLl/DaTYOB
# y2/43P+y00ewVn30M/jxzSKe4hWf6qCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCDjuSl54Zc4wlSd+tP9nobcgHC0vHkxqgatTKlinFTTMTANBgkqhkiG
# 9w0BAQEFAASCAQBdhaP3RbAEvJ8/XDZjKYWe3Vz3nECANqJcAaRvOjY3535Hq5tc
# ZZhuJxB+wsDCmuztwV8ZSoEnhl7BKWCEMxDhkw6X5cStACBw4f4XRMa/AeneY+yc
# weSMEOL1OsPFveeBxxY/WocXf8Wm3QtmfPJMi8t1vy7ywp8FWRCK0PKRsKNuWXil
# 5/fTk9uKxHqzQ2/mN/avApj4HH2y5Cn+UJxCTDYElUjTsiUnCel47vH35bmbHXm5
# wEUs3iNmwYytWUJlAgdyHrcABQqvAHgsjT+aGV+fpRRG0ELgir+ifqVWNOuZIkwk
# PcoZVrQLUiyurnGlmnEl/rSAIVaYz1rXQ2cnoYIXPzCCFzsGCisGAQQBgjcDAwEx
...
# BQAEICJOVB4zWtbxE9CRMSCeSaosfK1ZerQU8sMuN/QazEi1AhBWY3VL0ogazuaH
# S52VzHXPGA8yMDIzMTExMzE1MzUzN1qgghMJMIIGwjCCBKqgAwIBAgIQBUSv85Sd
...
# MsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEIPKpjCDMvhYfGsJQXbjT
# zV/7pWUyF0CU0EjQPv9bZP2SMDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3t
# dCLM0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICAFNZ
# QpSZtjx2kKYhbnj/Q1Rwmzd2dZ9wrbotwMifjjsmbjKSSJDAcv4G+cMXZUUstfwa
# Mq0u86/neK/xXWIul1yGb1H+yz/sAlzoG9p+sMLepT9uy2KmP7EaJaaP11r2AhWj
# Zy45g88LZ9SzXFkfMWQDnyMqRteLbyg48ziiIwVWMMeAbwKn1LvAHekuuAkjFkN+
# J/wqSd208BUg+7hpkUWGmBgsmn9Jzzqvp4ZRniMf3o4VjyD9l0yh3FGyyki58DyW
# Kbs+WRZFp9knbG+GYtKyQ8SXgvZFXSPgBOyFTz2KpK+M6moB1BgW5y+WIa492KTV
# rRtcPuLLa9o3sU5mrUsHUVnzvTel6q/JF4Ku8490pa2L5lXqqJ3NQSfvUY8ImJz7
# 8wG3EJCRfWEKJwIh6Qe53spjFYGOsnT2kX+/CAyUirBsZDM62avu0n8CXvTCdhvT
# wKP8jARFzK/w/r+0UY94c5qxGD5dnyNO0GrA6bd9W1KFc7jDG2uPwyt/wx+l/rGb
# ik+XjnVtGlRFUU0z7Si+Me9UHbmJ+ZQ1DkR+eHcMhoTnPB59cfnwD7M8iWePzSqb
# f5zHogyktUcPtXgfy4tAVoJuL1I7bYJ+iQIQS9TN+BKGG9qOqa+14XX20rpnN1md
# aMkJr7sBxUv3VECATe2MeuxGlDJ9Xi4l7NYhY8hg

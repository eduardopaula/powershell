...
if($verifyIntent.ToLower() -eq "yes"){
	...
	$templates = Get-CAGPOProtectionTemplates -Connection $connection -Credential $forestCredential
    $templates | Out-Host
} else {
    $isForeignForest = $false

	# Get and display the protection templates in the Change Auditor installation
	$templates = Get-CAGPOProtectionTemplates -Connection $connection
    $templates | Out-Host
}
...
# Store template
$template = $templates | Where-Object {$_.TemplateName -eq $templateName}
...
#OverrideAccounts
$newOA = Read-Host 'Do you want to set a new override account? Type Yes/No, default is "NO" (If Yes, this will overwrite the existing ones)'
if($newOA.ToLower() -eq "yes"){
    # Prompt for protected object distinguished name
    $dnOA = Read-Host 'Specify object distinguished name'
} else {
    $dnOA = $null
}

#AdminAccounts
$newAA = Read-Host 'Do you want to set a new admin account? Type Yes/No, default is "NO" (If Yes, this will overwrite the existing ones)'
if($newAA.ToLower() -eq "yes"){
    # Prompt for protected object distinguished name
    $dnAA = Read-Host 'Specify object distinguished name'
} else {
    $dnAA = $null
}

# Prompt for disable template
$disableTemplate = Read-Host 'Do you want to disable the template? Type Yes/No, default is "NO"'

if($disableTemplate.ToLower() -eq "yes"){
    $disabled = $true
}else{
    $disabled = $false
}
...
if([string]::IsNullOrEmpty($protectedObjectDn)) {
    ...
    $protectedObject = New-CAProtectedObject $protectedObjectDn -Operations Create, Delete, Link
}

if($isForeignForest) {
    if([string]::IsNullOrEmpty($protectedObjectDn)){
        Set-CAGPOProtectionTemplate -Connection $connection -Template $template -TemplateName $name -Credential $forestCredential -Disabled $disabled -OverrideAccounts $dnOA -AdminAccounts $dnAA
    }else{
        Set-CAGPOProtectionTemplate -Connection $connection -Template $template -TemplateName $name -ProtectedObjects $protectedObject -Credential $forestCredential -Disabled $disabled -OverrideAccounts $dnOA -AdminAccounts $dnAA
    }
} else {
    if([string]::IsNullOrEmpty($protectedObjectDn)){
        Set-CAGPOProtectionTemplate -Connection $connection -Template $template -TemplateName $name -Disabled $disabled -OverrideAccounts $dnOA -AdminAccounts $dnAA
    }else{
        Set-CAGPOProtectionTemplate -Connection $connection -Template $template -TemplateName $name -ProtectedObjects $protectedObject -Disabled $disabled -OverrideAccounts $dnOA -AdminAccounts $dnAA
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBNjF8XZvNXqtMg
# 0eOl/YX6b0HRCIombmz71qAhQ+V8zKCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCBZs/45XEW44s2xB7yBlcuy6xjkDqm9Rhtz37gtOYiJ0DANBgkqhkiG
# 9w0BAQEFAASCAQBpKUfeuEaVrFnimPQlQTRr38myaVbbAmOZYqCRJHrft2YyboeF
# TxuZ/92ilyFrYePMil90Cga08sfFh+OX7vi3gDsB6fI/UO7kKypCi9wFoHu34UEr
# 3jKi2oXGixp5cA/hT4CVgoxKiwAaT3mqcFAtvnd+owKDeR23aHlGLSi7WqTtydq/
# HKJXen0qr0mZJtfMsJgEX1QQMH5ZsvxYQuDTrMs+ZlJaY2Iuq++fRKIqSIUZybLc
# 6nrx+nKEkjKU8M+5vCoi3d6j3LyCb6T44hqZRk8DeBismXUj1rtLTebk4oErerzE
# 7e3GssZsCpH4LZ+cJGCKRBqrgXaPME25XpCRoYIXQDCCFzwGCisGAQQBgjcDAwEx
...
# BQAEIMwUC0E36/aZAdOKEK8m8Ai2s5slwaqKlZGI5PZdSe9PAhEAifBv96clg5Ki
# 5JRUPpeOsBgPMjAyMzExMTMxNTM1MzhaoIITCTCCBsIwggSqoAMCAQICEAVEr/OU
...
# DQEJBTEPFw0yMzExMTMxNTM1MzhaMCsGCyqGSIb3DQEJEAIMMRwwGjAYMBYEFGbw
# KzLCwskPgl3OqorJxk8ZnM9AMC8GCSqGSIb3DQEJBDEiBCDEsKJpUIEeuLvMMn01
# aX5Gl+oZ9CSN0FrT+gTK6PRG9zA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDS9uRt
# 7XQizNHUQFdoQTZvgoraVZquMxavTRqa1Ax4KDANBgkqhkiG9w0BAQEFAASCAgAa
# OYfW3mdLXauN4Wu2yFK6UOm5BrqdGxenigjLRLepw5vMa0TsSWqKSB6n9T3iUbn2
# Un7x0iMnEwmqwxSMK9ZM5sWZkxR/a3NQu1a2gmsYlmzbzOS6xWjrvaEdmXTWEXug
# Sz2sdVLwXWOafw8IX4T0+IwSY8Yg0elW3cShBKXvFX+tJVF/7ifWqzvariJiHk5x
# QzpFYZm12BO2YSIlh08w4chfSMR1qkpE4gUy691Ze7mNDLTEGid3PpkUODaLXh2c
# gkAw54fsh7qHs5v2f+E6Xq77z131j3/0FLS1DpLOKLD4F3rZQkEFv2sBoIJNdDMf
# nK44m50QJaJr/931zF5inrH1mBJAV3BmIkaj0sPWu8z4nmrCdpr5m0zDU0Gdh5qZ
# tjwhGoEWOHFDYe7WCWEqvLDIjd/4azLt5/Iujxxjux2Eb/1hmZ0ZqDt4hv/pyeqQ
# U8TKg/ncyoa4IrCLw2mi9m7t0dmIF/YeyGE/VQH2NQm3x1GXGsog8u6zkyT3ijBX
# QcTm0OjrMNotoeECmZd7kUdH/Wlo1EEPB2g+/PbGFSpm6BugIxrmIPukBS/YYejz
# H/B64hvNIFwcN4xrfhhuLkOD/lcW3IPdwjbIP5oplrSYULauT2KVqQDEjOCV5Ykm
# QguFnSOpO9bE/NpAfWRLJ1Zk331+VZcbN8uvXsf3mQ==

...
$subscriptionId = Read-Host 'Specify the Subscription ID of an existing Microsoft Sentinel Event Subscription'

Clear-Host
$validInputs = @('1', '2', '3', '4', '5', '6', '7', '8', '9')
do {
    $input = Read-Host ("Specify the number corresponding to what you would like to edit`n" +
    "     1. Sentinel Workspace ID`n" +
    "     2. Sentinel Secret Key`n" +
    "     3. Enabled state`n" +
    "     4. Notification Interval`n" +
    "     5. Heartbeat Interval`n" +
    "     6. Batch Size`n" +
    "     7. Subsystems`n" +
    "     8. Allowed Coordinators`n" +
	"     9. Additional Office 365 and Azure AD event details`n")
...
switch ($input) {
    '1' {
        # Modify the Sentinel Workspace ID (also requires new secretKey)
        $clearNewValue = Read-Host 'Specify a new Workspace ID for the Log Analytics workspace enabled for Microsoft Sentinel'
        $newSecureValue = ConvertTo-SecureString $clearNewValue -AsPlainText -Force
        $clearNewSecret = Read-Host 'Specify a new Primary or Secondary key for the Log Analytics workspace enabled for Microsoft Sentinel'
        $newSecureSecret = ConvertTo-SecureString $clearNewSecret -AsPlainText -Force
        Set-CASentinelEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId `
            -WorkspaceID $newSecureValue `
            -SecretKey $newSecureSecret
    }
    '2' {
        # Modify the Sentinel Secret Key
        $clearNewSecret = Read-Host 'Specify a new Primary or Secondary key for the Log Analytics workspace enabled for Microsoft Sentinel'
        $newSecureSecret = ConvertTo-SecureString $clearNewSecret -AsPlainText -Force
        Set-CASentinelEventSubscription `
            -Connection $connection `
    ...
    '3' {
        ...
        $newValue = Read-Host 'Specify a new Enabled state (true or false)'
        if ($newValue -eq "false"){
            $newBoolValue = $false
        }
        else 
        {
		    ...
		    {
			    Throw 'Invalid selection. Specify true or false.'
		    }
        }
        ...
        Set-CASentinelEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId `
            -Enabled $newBoolValue
    }
    '4' {
        # Modify the Notification Interval
        [UInt32]$newIntValue = Read-Host 'Specify a new Notification Interval in milliseconds, 0 for continuous'
        Set-CASentinelEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId `
            -NotificationInterval $newIntValue
    }
    '5' {
        # Modify the Heartbeat Interval
        [Int32]$newIntValue = Read-Host 'Specify a new Heartbeat Interval in milliseconds, 0 to disable'
        Set-CASentinelEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId `
            -HeartbeatInterval $newIntValue
    }
    '6' {
        # Modify the Batch Size
        [UInt32]$newIntValue = Read-Host 'Specify a new Batch Size'
        Set-CASentinelEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId `
            -BatchSize $newIntValue
    }
    '7' {
        # Get the subsystems
        $subsystems = Get-CAEventExportSubsystems -Connection $connection
        'Available Subsystems:'
        for($i = 1; $i -le $subsystems.Length; $i++){
	        [string]::Format('{0}: {1}', $i.ToString(), $subsystems[$i-1].DisplayName)
        }

        $allowedSubsystems = @()
        do{
	        $index = Read-Host "Specify the number of a subsystem to add. Enter 'all' to select all (or press enter to finish)"
	        if ($index -match "^\d+$"){
		        $index = [int]$index
		        if($index -ge 1 -and $index -le $subsystems.Length){
			        $allowedSubsystems += $subsystems[$index-1]
		        }
	        }
	        elseif ($index -like 'all'){
		        $allowedSubsystems = $subsystems
		        $index = $null
	        }
        } until ([string]::IsNullOrEmpty($index) -and $allowedSubsystems.count -gt 0)
    ...
    '8' {
        # Modify the Allowed Coordinators
        $newValue = @()
        do {
            $input = Read-Host "Enter a coordinator DNS or NetBIOS name (or press enter to finish)"
            if ($input -ne '') { $newValue += $input }
        } until ($input -eq '')

        Set-CASentinelEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId `
            -AllowedCoordinators $newValue
    }
    '9' {
        # Modify the IncludeO365AADDetails state
        $newValue = Read-Host 'Specify a new Additional O365/AAD Event Details state (true or false)'
        if ($newValue -eq "false"){
            $newBoolValue = $false
        }
        else 
        ...
        Set-CASentinelEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId `
            -IncludeO365AADDetails $newBoolValue
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBqmMgNTuaB6bDt
# yR0BxAFSK3/mtHjFejnIo+O8FHZzaqCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCCQVmNNygIGg35Cp5UaN/XHgjFbPsGrfVE2bC6TKIEH0TANBgkqhkiG
# 9w0BAQEFAASCAQBjN48G6f29I79qXz6pqKotfTqbjrGd185LndiXMiLKwsqchkDX
# aLGD28jRgIvbzwVWcOlnNeQauJyGhdU7A1Ky9cT0D74O0qYnuBqvaYJazOpkIWG0
# Nv1XK0a12wexJdDfd3PluVpWDQiMFlU68ckprMkRrR0PwE59Y5W8W+ELYjQ5tKNc
# bc2QEGcFisMdUzf21DWVllMwAcqfzAyopIkVVI2GhmmZIwG5oGA3SqhCMwAqNQCN
# SseOIR6La3KM5KMLxbDrcJ1PMVacAqYFB4Wlh2CK+0QfjNXYsRamqE9JkBMTZ5ov
# K4UEeLoxzqkEm7r+nUFXD8teK+gQU2QoE+WWoYIXQDCCFzwGCisGAQQBgjcDAwEx
...
# BQAEIAo5p6G5ERnExdMIvAgln/mqAtFbrmG+qS3TRP2xdKoeAhEA+R7wscASMetH
# mY0Rd53mKRgPMjAyMzExMTMxNTM1NDBaoIITCTCCBsIwggSqoAMCAQICEAVEr/OU
...
# DQEJBTEPFw0yMzExMTMxNTM1NDBaMCsGCyqGSIb3DQEJEAIMMRwwGjAYMBYEFGbw
# KzLCwskPgl3OqorJxk8ZnM9AMC8GCSqGSIb3DQEJBDEiBCCXt2bKBUifEBPHjlmn
# fanVmsn4Sh6gpiQZqzM6exEhhzA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDS9uRt
# 7XQizNHUQFdoQTZvgoraVZquMxavTRqa1Ax4KDANBgkqhkiG9w0BAQEFAASCAgCK
# LawDklbYI350WJN0lp3Uavqm7N7NpC4fiSPHARApb1OjRyWa7lIlbVRr1NOC9c+4
# Z0gSrNAT/67YzmCilg4ek5PzaWyctnHrOK36Y89Foxl4UQAb6TyfUPZvYXRLWyPz
# dEORnxdyDfro76j2mKKPp0bRKjMMKGqPxhfUYm2GWoP6mkAPTsh6qGaLH83F3aRO
# LIBZSaPbZhvkBK7AVnGFvf4/aPiYtJyDPFngd1xX1rnbUrWqNxC5lI2BCv8b8jbS
# peu8LC6Zmoiw+tgtp6aV31P8n5Ty7K5J9SXYee87og3Yn02S0pqM/v/8S5pHJfx0
# KSwv/SLnC51jNlcvrzeXEuEIfhxQR5a/4boO13xcoCHIndiIe0cEgieqexaoKKaS
# HkVtMXMIUghSQqfuVNsaSz1eiwDugiOSqWKC4b8qR2+Wk15p0elhhy97i0Qw0dme
# c3aW9sHGX0Ej0mj+BGM05QBbu0Lxvyo8I0QNHjrBb2+431rtEa4bkjuqiKvrWdNu
# ucEHmzjLEd3p+uMwBqPTNpZX+gqCBR2pbeVlhGQDOvUycIFiRhN2MmijTCiM7DMK
# pK1KWItKOcG9WA3mhuAIr3qkj8vfdX5aMiZRR9cFjT5w1SA3fl/3lOll0BtluM2b
# Uyyy+7YxlVnJS8ndV8e+IofQJ3RqTWHM9bdo+82iJA==

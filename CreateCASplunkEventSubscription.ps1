...
# Prompt for included subsystem(s)
$selectedSubsystems = @()
$subsystems =  Get-CAEventExportSubsystems -Connection $connection
Write-Host
Write-Host "Available subsystems:"
for ($idx = 1; $idx -le $subsystems.Count; $idx++) 
{ 
    Write-Host $idx ': ' $subsystems[$idx-1].DisplayName 
}

Write-Host
$subs = Read-Host "Enter the number(s) of the subsystem events to be forwarded (separate multiple entries with commas)"
$subsArray = $subs.Split(',')
for ($a = 0; $a -lt $subsArray.Count; $a++)
{
    $idx = [int] $subsArray[$a].Trim()
    if ($idx -lt 1 -or $idx -gt $subsystems.Count)
    {
        throw "Invalid subsystem number"
    }

    $selectedSubsystems += $subsystems[$idx-1]
}

# Prompt for the hostname and port where the Splunk instance lives
$splunkHostname = Read-Host 'Specify the hostname of a computer running a Splunk Enterprise instance (https://[hostname]/)'
$splunkPort = Read-Host 'Specify the port used by the Splunk Enterprise instance HTTP Event Collector (or press enter for default port 8088))'
if ($splunkPort -eq ''){
    $splunkPort = "8088"
}
$splunkUrl = "https://$($splunkHostname):$($splunkPort)/services/collector/event" 
Read-Host "Built the following value for the Splunk URL: $($splunkUrl). Press any key to continue"

# Prompt for an Event Token
$eventToken = Read-Host 'Specify a Splunk HTTP Event Collector token'

# Get the starting time
$startTime = Read-Host 'Enter start time (or skip for default)'
$dtStartTime = Get-Date
if (-not [string]::IsNullOrWhiteSpace($startTime))
{
    $dtStartTime = Get-Date -Date $startTime
}

# Get the allowed coordinators
$allowedCoordinators = @()
do {
    $input = Read-Host "Enter a coordinator DNS or NetBIOS name (or press enter to finish)"
    if ($input -ne '') { $allowedCoordinators += $input }
} until ($input -eq '')

if ($allowedCoordinators.count -gt 0){
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD3Nb4DI3SKdLKd
# xZZwWicsbcRBMHoDNN5Z2RQ0kTWMU6CCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCDyEWsSVT3FFmLe4+NGbotHUFVHuNfCo7RpQXuqvQ6QNzANBgkqhkiG
# 9w0BAQEFAASCAQBH4L8uMk/6NKuxeBAhYS9zsTF4/8scBC+lJwYOtK2U8Kl5/Tms
# w6yVH3nAidPtQcGxxGrqsgEvmPYNZGY11WxcSf0Ss8Fqq9LZ+eaE82enGIQ8XMXs
# jvJqWxuDzHwS9ZF6urqwnMYt0BwUsmWxQrlDCLwmbiS3rePDENpSZllpCnZA4ctB
# e7YJwZe/OAM92BKgLaiu1U0HXUv1f1WEMQJd1JqtCdBjk9yWHoHLmw4GnYkiutaJ
# ZER+ibOBlCVh6Z5eF7JWCnful0RJ/xO9U5eSTkEnl1U98+KLttEgLZL7etSQlZ2N
# p6maVJ85yCRbQqGoaIeK+D4Quvb/KhrHE1uOoYIXQDCCFzwGCisGAQQBgjcDAwEx
...
# BQAEIG++oJpz24HRSJwW5SrAXUotNe/rVGCmGV89zEy/ivG/AhEAvB/isGsu5mrd
# oxocN8nJ9BgPMjAyMzExMTMxNTMzNThaoIITCTCCBsIwggSqoAMCAQICEAVEr/OU
...
# KzLCwskPgl3OqorJxk8ZnM9AMC8GCSqGSIb3DQEJBDEiBCABBlBcWNttrkMD4bkT
# prxyIR4ee6lodQcdf+dbuskNUDA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDS9uRt
# 7XQizNHUQFdoQTZvgoraVZquMxavTRqa1Ax4KDANBgkqhkiG9w0BAQEFAASCAgBG
# 94kZhcaMEO1rqlCPhlbrG0amH4EUE2jVZlmelupPmJlZZwL3DYzAazqQIQJU1ytn
# KDaOv7arm4FQZEgGMDIiZz+G+7K1dd/fDB096EOlnnWlFfMm2u4nnqiAf/R0fQ8Z
# NZEiJVlsEc3PSOsOnt4TpYqVJ4ERnsREo51y/hiK9kCVNtyWg8kjbPuneWTlhDR4
# ozUk///7z3zjjlyRjdaVixTmHFBK9Pk8LrNvHEY73MXadeP+1zLHIhRn4NLJwBZ/
# O48BZ7KSd9TYmFoooL3vVYSomDJikp4sdAdgY+sVCKmL1EsC2eSrUW9sBOUrETZT
# la4etVvcA5m5nc9tf9cYI5ZQQli+zCBKjuw6F9XJpbec2d2EliuEhBftc8bQ7Bq8
# Q3ndqgJkPyI1dqMLpdYSBP4YQ1uLJAbDFvn+6tXhKenYvJcTM86drU6j6Lq2mwlf
# uv3yahs08gSyPxSTD0um5cYV81wPFHcWoqsJYCTQUmZfr1Ag5jox7/DrwJ1oaL8h
# 8/8z9rffM/N4eidCx8/5GSmvKtvuQtNoINa74+9e0qM14ytPLfZuc3csUocOdSzr
# y/H0uJ2C9h+EHKBLZLPQxW2PX3Tv7al/ATWYVTo0VRylKbbzeOGjMHwwVTimmb7l
# F5yJ8486RQHdX4fdT7yHsSQEPMA/CGEYaLiYbmrdgQ==

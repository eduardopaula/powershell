...
if ([string]::IsNullOrEmpty($overrideAccountDn)){
    $overrideAccountDn = @()
}
...
if ([string]::IsNullOrEmpty($adminAccountDn)){
    $adminAccountDn = @()
}

# Create the protected object
$protectedObject = New-CAProtectedObject $protectedObjectDn -Operations Create, Delete, Modify, Link

if($isForeignForest) {
    New-CAGPOProtectionTemplate -Connection $connection -TemplateName $templateName -ProtectedObjects $protectedObject -OverrideAccounts $overrideAccountDn -AdminAccounts $adminAccountDn -Credential $forestCredential -Disabled $False
} else {
    New-CAGPOProtectionTemplate -Connection $connection -TemplateName $templateName -ProtectedObjects $protectedObject -OverrideAccounts $overrideAccountDn -AdminAccounts $adminAccountDn -Disabled $False
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAgfWEL82Q5B3wf
# 3woHN7pMu+8lDPlfVhRETWuLJ8O3hqCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCB4t/Yn7K2b+ZgL9IiQngg6SdoxJkW+CgMGtkb1hMLdijANBgkqhkiG
# 9w0BAQEFAASCAQBSMmlh7CliezAWuhKS+lFS1zBfQzJJ/QEHHiftgwzt9PU0esGq
# 2XNCIc0aQXNyEbFvFqaKHO/PKqudgJVR77yUFlNSmg1kov+jgxG6KntHzga/H0Hg
# 389IfMExiod3CcxR0M/IekZ42d+O5rneeD+kEvdbAkyGTDmTHEgMIISE/HQHCVUE
# jSsYtSrSDJYHbcbooofnxDFcIgHL0XqB2xzhdUCn51h5X1ZiPpcZd2d9VzL2bVvC
# lMwITRUZTUe6ZsRV0g8wg5meoqQTZJ+/KMVjFpmA2EYvZM0HspEZ6EjlEHigCrKV
# Ef8cSYJtPMoe2+D0sWOGenjDVjqDlbN28o1+oYIXPzCCFzsGCisGAQQBgjcDAwEx
...
# BQAEIKswkqGliSf9iXFtT3ufQ9gdsy7UiBuFkonU5MquOxfVAhBCaxy6fPXgmVjz
# 7Lefa6B2GA8yMDIzMTExMzE1MzM1NlqgghMJMIIGwjCCBKqgAwIBAgIQBUSv85Sd
...
# MsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEIA3XRrg0NpAUf7aTqVtF
# Hqu1O1BuvzGVGNQXNS1cbvqBMDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3t
# dCLM0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICAIzn
# TX4EzOyBu/rDtnNPQugZtfSHMu3g4cuWxgXVlChHDPAQps8ST6vWddhLnzUfjI8s
# H6m6+ET72eFLtjnqExScpvFZACk5PxBG7DUuaIzwlSF37CKI6dcedCaEHeUyjFNq
# SUvV+GdWQozTYBuLfRscuefUnP9tPbQQVJ9yqQ7Q6gYW4xfz/I7/mK2dWWB238jq
# pXxUYpIZK24QsJ8rQTedw4PMavcwUpFr4NI1ygjMga0poi7wExi1UCbVjtN0Eg1+
# StNiuv5T33F6RWzp91wjm/XTcnhnpG5VG/ifCPKum4fv829lhPJQqVFlarpmcVMA
# 2syiXQw/cJIICVUJHY5IGt9iucwyt/gqyKjNGeS7lF/mTc7z2Vb0QYH1hn05FHf0
# 42gtTjZMt3jZGAd0mkJdxzDBgyMMYicS+NyoKWj+d0A8KAvIknf32Hop54nUtjLn
# Ml5Baa4URfHKOGZdf8jR+cRF3yxKBO8lXS+DrIatp7BMDHkAY2RWNg0hb15DIlSw
# c2ZbofXP7Ym+T19PcxnYYb29bKy2dL0fGJjRllmVRE+kTyshO8V8EmJfP0+Kt5i1
# dQVYyjpdTCxcX2jalUmg262Hj/oTHqOmxQDH+FDfwVawFAf8RgjtYUPrrnr/GNe8
# ttTefEJWEIQopW3HkGSiNWrziwqg4C2KzOrKXrCn

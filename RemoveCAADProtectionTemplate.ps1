...
# Prompt for whether remove the foreign forest protection template
...
# Get the protection templates in the Change Auditor installation
if($isForeignForest) {
    $protectionTemplates = Get-CAADProtectionTemplates -Connection $connection  -Credential $forestCredential
} else {
    $protectionTemplates = Get-CAADProtectionTemplates -Connection $connection
}

# Prompt for the template to delete
# Run the Get-CAADProtectionTemplates cmdlet to get the protection template display name
# Use the display name for the templatename
$templateName = Read-Host -Prompt 'Specify the name of the Protection template to delete'
  
# Search for the specified template to delete or exit script if not found
$selectedTemplate = $protectionTemplates | Where-Object { $_.Name -eq $templateName }
...
if($isForeignForest) {
    ...
    Remove-CAADProtectionTemplate -Connection $connection -Template $selectedTemplate
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD1IfvLA4NSGnH/
# lhI7dSIBC0opcOpEq0xXFltuiGE8hqCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCA0ME+3NtA+fiduOMz52NkiGWFsZ83rE5uGYhknScZTKTANBgkqhkiG
# 9w0BAQEFAASCAQCfbPCU0LszTWEun7Uxt5lpj/sLFzU9eIkJk2tX5Lp9kfQy3FBI
# EUqLRgZSUFC9YQ0mnvvJlJZpPXfD6UGOiTIV2uwjYYuVZ0BVLxoVe5b6OnygTEFs
# MciPLbcAnFnyC1sIJHhDCbfts0oK1K+o+6XIoxyBGHPpnKg4YoP6AAm84VR+39Oh
# h27dizkyUITAI7KqaJpjIyckhwg9l7ExWjNEZ0dtHNS/LUaP59cR/ROjcqGEJVBR
# dAIZ6zsK5svkeI5Kr/9lKbdNE7aSmO9qZG+nZu7/+scIDAl4ymvwaW7BqT16mauv
# vNrDyHGp5NhH848Y00m1H4fIPOPF+VzRNh5voYIXQDCCFzwGCisGAQQBgjcDAwEx
...
# BQAEIBf0WlBlBokXr+iiyJkuc5/7sSZnf46+oB0Kb4i581vaAhEAwB8KU744r20N
# MpLPP6mUIhgPMjAyMzExMTMxNTM1NDRaoIITCTCCBsIwggSqoAMCAQICEAVEr/OU
...
# KzLCwskPgl3OqorJxk8ZnM9AMC8GCSqGSIb3DQEJBDEiBCAq1nOzfphnihrO1aax
# WjB10RuKbB5gaAf2yGfGbKBIGjA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDS9uRt
# 7XQizNHUQFdoQTZvgoraVZquMxavTRqa1Ax4KDANBgkqhkiG9w0BAQEFAASCAgBE
# gcoEkQFU1QqxoDlwZijRo4mPhScEqPO0JePormztZVJGK8Z2uSeJFQqtfRYhemcW
# bympNSEtIWe9LOiobsJwIuydqiAniCy10kx2dN6OHDao/GxbVUf4HcLBuNlO5sJC
# wfJKh+v29/VuJZNjv4K1k81b4ouLK7cG79xTQGEs4g4h+aIwaLA6A4eN2RlnyUaf
# m/F3JxweQ4D9+igyfeQatr2SnrvRYpz2ukrzGQoPjwju5nlU68Zyu/CH6o0DSacE
# ZipDd9EYT2EfrQBsFJl31GDuvqVhspBLdI5h8NAAL7WgtD57PoHw0KzhOGvr8YX+
# L9RRTR/X2fstjz1GbklOF3V+6TX4i3lcGgiQjQuHNVRFC0pF8zRLTQpJujuzkg9u
# 4HFogqW5n5f+zaIMn4WL8i92h1RWHRGnGRhvWnW7Wn4m+UIEgycAFRH1wrQHxY1b
# LP9SO/qa102kDqld0oMV2fJ7JRUa5PqtKsw6PuVE/9/I4QMrhkKFq4VsxGnKkerv
# 7iot0XoppvDM1PkM0q3HtNC87EsLN92e5ta7Iz5Uz4Snbs9mDieMn/Dyo1IWLdpT
# /i9WrN0ApcJtoAbX5xZnuI4ol2VdqimeYnOeyD3/X1Jv3EZzgp/swCOj88ADkdUO
# UkQQdKqtMPA6I4R9HVJKc2RgM5Z/HamXHMWd88pOwA==

...
do {
    ...
    "     1. Get all Syslog Event Subscriptions in the installation`n" +
    "     2. Get all enabled Syslog Event Subscriptions in the installation`n" +
    "     3. Get a Syslog Event Subscription in the installation with a specified ID`n")
...
switch ($input){
    '1' {
        # Get all the Syslog event subscriptions in the installation
        'All Syslog Event Subscriptions:'
        Get-CASyslogEventSubscriptions `
            -Connection $connection
    }
    '2' {
        # Get all Syslog event subscriptions in the installation that are enabled
        'All Enabled Syslog Event Subscriptions:'
        Get-CASyslogEventSubscriptions `
            -Connection $connection `
            | ? { $_.Enabled -eq $true }
    }
    '3' {
        # Get a Syslog event subscription in the installation with the specified ID
        $subscriptionId = Read-Host 'Specify the Subscription ID of an existing Syslog Event Subscription'

        'Syslog Event Subscription with ID ' + $subscriptionId + ":"
        Get-CASyslogEventSubscriptions `
            -Connection $connection `
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCoHL2qqmRe9vhz
# QOKo9oBuRCsT2BHP0105afU7dyZAjqCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCC9IxmVZnkjPTN9t8KnqfldCyRqBj7dJzL2but+KHbrWzANBgkqhkiG
# 9w0BAQEFAASCAQCDvKv5N2BJZEN70N1Td9xGQUzOuuVj5drLOJLkae4rFHVeTZta
# 20M0cGfGcSw5ZR0eSiQOkZiyXmcWjxaVNmWg/Z6nzRbGqYGZ8QbotOZCufBi9yHK
# DCnIaPGHCYlUglN9kO0xmbBmrDVVvPxhovpjVyAdHH+Joi9oc3bbudVqi5UWhenX
# ZobEbxRKt+XEt4PRIor3EuQoW4CUNoiAVPdSOzYX6S0RjjBJdSBphFtdYhA7mHVH
# S317e1XtaBLiNFxb/556iCuc7o9zqX9AonXdEUBf2VGg5bCTnSsYkkcfQmjaVqIR
# a9QtBqu+Gsq3RuokUu78s+807G0AEyKjqsnDoYIXPzCCFzsGCisGAQQBgjcDAwEx
...
# BQAEIKBTF9WmBNmUPk6Y9p0fMimk8++FpklgrscBkIDpPFNDAhAwUr8A1SM1GfV9
# IBIySd7PGA8yMDIzMTExMzE1MzUzM1qgghMJMIIGwjCCBKqgAwIBAgIQBUSv85Sd
...
# AQkFMQ8XDTIzMTExMzE1MzUzM1owKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQUZvAr
# MsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEIK8CG8BgflQjJacTlcEO
# qRijms6g8Bnv23HOxMzlODG3MDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3t
# dCLM0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICAFnx
# bhxSixdlxdHVleKFKM1i+1ofijfWKmXWm7OvIu205tEMFRekJ5pZ+CpMsxhy5Gex
# GNqe4DCn7XhyAUbrghkCwOxdtkIqW0jKdw9LuHf7+Q+Ga/caIptfqZbGIT9BeNW+
# FIdgcdn1bLb76hjuHnDo049ooEpzpUahEj2cFtYQ3tPX2CSa6oPL45XdjfGBaWSG
# 2zISLJ74PB8vKaNuvjYSPqbXd65VUa5JF3XsLGeo4BLXoj6ITJxLq6rN+022EUIx
# TSZ96YcDdBa3nRedu60BN7xmJuKUMz3tH07a2dIKVppjHZrHgzuZj+thmr/XvaXm
# GQvHt7uQQD18Q0R9oUkhTYO7+QpPjjmsEERhj9HTzCOucFLqo11UX/WOU1m0BvyD
# bcE9CnJyDkheUGHDYiXfDZhKvJfiKWLfPOdcz1evoGpuYeay1avqP/q89SF3q6hs
# aZtYaHn/2OpIlZ8vWS3KIdecYt1S+WA/l+Dju3NIiLDny5D2/A2VB1+aUNQKSLi5
# HvdgtcXc/1FSZc0cD2EwEtM3TJFSW75J29uafVxWuELX8A3ENLMsN3jpH0whoPex
# 8I8Yyoint80K9zfCDi9wntmSnsHm0luhhCkmBZtkJsGssCgcWA2ne8+B8kzxAkJ3
# zyb0M1pDdBXoGRVxx+X94vGAJOt+2AOeANSoLahA

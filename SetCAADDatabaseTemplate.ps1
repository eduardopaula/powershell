...
# Get all Active Directory Database templates
$allTemplates = Get-CAADDatabaseTemplates -Connection $connection
$allTemplates | Format-List | Out-String | Write-Host

# Template GUID. Change to your expected Guid.
write-host "Here is a list of the following AD Datatbase Template IDs" 
            foreach($r in $allTemplates)
                {
                    write-host $r.id
                }
            $guid = Read-Host 'Specify the ID of the AD Database Template to Modify'
             
#Filter with GUID
$modifyTemplate = $allTemplates | where { $_.ID -eq $guid }

$templateName = Read-Host 'Specify the name you wish to give this template'
# Modify Active Directory Database template
$template = Set-CAADDatabaseTemplate -Connection $connection -Template $modifyTemplate -TemplateName $templateName

$template | Format-List | Out-String | Write-Host
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD2vRzrv+rjNMjY
# E2wHFwNeo7hk2WwbW2xOqHXgzrzjx6CCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCBd7wASA3iwGuWPk/oVlu/N7OdtcfybfO0v55QJHDuASzANBgkqhkiG
# 9w0BAQEFAASCAQCyfWQKt36fMnlHvPLIqsxBJd0lxUsVgH2St2+d7KMLo9vndl4V
# hSTCbGk3P5o+z5yxL8hWNanck6ukho4HI7POn78tbBiBaTnlMqWEJgurU0bjVQbs
# uZwD4yFtFaZZz+ymmibaMN5itYSg6s4o1bARNCoWM1tN2bN3aCNQYNaWUwlhBsAf
# Ab1mJNtvFYfyh4Ygo+d0SY0oX/l9OjKADdgEKArPioeAXc4nPTqabOoaITwHVWR8
# K7peQO/0LBsJRrzLaNA2GOVIVdBWhrQncf5QmSyJQIhEwYPr8fn1enOCcqX9EjB7
# 2+6GTp0SEtpHWMJJYU6nsJ91lzhOeo0E+QktoYIXPzCCFzsGCisGAQQBgjcDAwEx
...
# BQAEINpNYj4v5uhtj83dNmMeZDGL1nkBDpqk13HFfEi8pGnEAhBBvcKyGVmBEcd/
# FbhitsMLGA8yMDIzMTExMzE1MzU1MFqgghMJMIIGwjCCBKqgAwIBAgIQBUSv85Sd
...
# AQkFMQ8XDTIzMTExMzE1MzU1MFowKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQUZvAr
# MsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEINxROOKmpEE/Xj4c1bsb
# 7bi2rUXoyDUIV+Du92RAklw6MDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3t
# dCLM0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICAIY+
# /k7Q0kINrJzQY/oHzcrNIojp4PJ1AIdNxInwEajbUvkPmN9WN5ZShfKN1Hhlueo6
# RO8WqBC7dU08du2Mc3nS20xec+7Nyzgi5Z4LAGEBJwLFfyW9M7a3Fj0oP8gulHIz
# slNBEnQH6Tze85BKdUx6l6PqqUnHjz5uQOnkH3j9QFrppQr10V54yY2ujprA/wZe
# m3gHdTB5QDKr4bHYFucCW8jbpBVl+VmV6JRUFP1bF1yif6Hvd1Z6aAKw5KjqnK6s
# 1xCIHYDXpIMfN+PHwLstjq3StXyy3ugSlua+Coliro02x7xYKY4It3WHiCJDCFc9
# OUEFOcLoIPhT3oCCEiXaRX8nXi81pTNM0IH2Dn1uVgpCgtnoh7Xe+f/ilaFh++Qx
# VnyixdkF9hFDiEehc3F2EJa7br6TlkNeMW6mWk6EOVdMTAL3a7kZwrOrMImDqyY5
# wR6lzhlLgmyHC2YQ/EDZYtFmhuvQsBn6H7cyfknoszaaCo2iPtbtzfcxTQ6eiUsv
# fuD50WJ9NDa53Ma/WXP8kQs22hZW+v8gmCSERINHtsV9srZAowEXsAs1oY1BDlxQ
# DxJU9TirnzxJlmtlmp2zRGymNKFA+Z1sS/e98808DJEvnt43VchkUmIIc48AAvA0
# AkfIFs9rYhZih3z67LigjJDv9B0WkPfIE0x3lUmA

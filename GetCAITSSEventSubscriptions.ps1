...
do {
    ...
    "     1. Get all ITSS Event Subscriptions in the installation`n" +
    "     2. Get all enabled ITSS Event Subscriptions in the installation`n" +
    "     3. Get an ITSS Event Subscription in the installation with a specified ID`n")
...
switch ($input){
    '1' {
        # Get all the ITSS event subscriptions in the installation
        'All ITSS Event Subscriptions:'
        Get-CAITSSEventSubscriptions `
            -Connection $connection
    }
    '2' {
        # Get all ITSS event subscriptions in the installation that are enabled
        'All Enabled ITSS Event Subscriptions:'
        Get-CAITSSEventSubscriptions `
            -Connection $connection `
            | ? { $_.Enabled -eq $true }
    }
    '3' {
        # Get an ITSS event subscription in the installation with the specified ID
        $subscriptionId = Read-Host 'Specify the Subscription ID of an existing ITSS Event Subscription'

        'ITSS Event Subscription with ID ' + $subscriptionId + ":"
        Get-CAITSSEventSubscriptions `
            -Connection $connection `
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA/eymouiqkMFZc
# ndG77UuP+RgaJJS156B76uI2NyHc9qCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCA6zM/pfYTwWTdlsyrVBPrKQXIOUT34821hpxnlLI0FhDANBgkqhkiG
# 9w0BAQEFAASCAQAzjP0zaGICQ+hdZY27uzPnfxhW+pabhXomyzBW+KlWPZNVAY+t
# dtKk4hDdry4XlFE9BA5Lwqz8EYO65LbbwWUSAsczpDXTv2PkMk/UJUiLZQlV7hI1
# gUCytuWdfw7DNPyWHN6DkBmJ3HaI8Q+IG74TqwekuiHLWdPvE57WYmhkbhoIbM0Z
# 7RbJRtlTi/0PGD10o+CtDthqmB5zLu7FsC8kVxPsSFP9y03d6FL3O+E6DGdrNc/W
# Sx/sYsVCJeXG3Y5vitp/FCRALbVdz67t4aTHmeD2yjbdJTTMfRfzEeoIysVMbQvh
# hx0L+VbQwmFjQSqwWQIQfKS6JbRPQDlhYRXBoYIXPzCCFzsGCisGAQQBgjcDAwEx
...
# BQAEIEOOuCWRYcGXJHCNpJglFORV5uvH1ezKZvcTSES4kDfcAhBt6qdyRiau3H/L
# KaJkXXUOGA8yMDIzMTExMzE1MzQwN1qgghMJMIIGwjCCBKqgAwIBAgIQBUSv85Sd
...
# MsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEIFCVsIavcONsb49Grnmq
# qawE7MeRSPL9CPOcba7oM6+6MDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3t
# dCLM0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICAF/F
# dRsRIbSsa+LJdImPofzvnMY95o1hAut1m1LDpmpUs/KX3rGTVgxuBZ4vzATEKVj2
# a2tZLTRyvruaTAj/clYqElet1gWGQhdqfP2baMS5aetK/4TCUE0jgEpNnjPMrpCS
# In3sawMz9fTK48W7MqoUCDQDJ3ZQiLcIAYDi2x4fWHiQMc0zBWkAvJ6NAMvpZdS3
# 8K7O58ViXWxeYf2vuLO2wEmRb4vgoUo94EbRaTtHMCflsnTFb2KL9VnswsyDi2gM
# 2IrE8FmT/bqSuWS9iljCn4Uy3pqSY3E5oHWo8JAjpNoLaa2Avlk2OOWSXYpAs4Xc
# wZhYjpd/WDv8/EnBlUbnuF0qsV0Z/OhfikuWxqRAzrUauFaivqKIxO2b01nrDIXU
# PI0DN9G5KyazmalLfN5KCr60xNhIu4mUd6vQDB06956it9Z1DQ/HM6tOdCd/vnx9
# Cb+hEv3ixjRX6GjnqUMWJsZHW5WzhmtcschW3a8SmiFpibc9AxdQ0CD5YFiRYJeQ
# 0XbjiiKbS6clvt2h/nJj1Qyh3OcmBaz/9Cx3uK5Buuu9rCXnjaDi2Ui/JTEWDiJE
# r30Nm0n0HzpWGwxgTV9PQM2BMF4UjToYU/6m1Zoh6VSvGCAkKf00hZjggpmlcd2w
# boBnAlDb1eL2HRV371FTo8XqdrSpxki1UN+vVU+c

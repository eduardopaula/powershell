...
do {
    ...
    "     1. Get all QRadar Event Subscriptions in the installation`n" +
    "     2. Get all enabled QRadar Event Subscriptions in the installation`n" +
    "     3. Get a QRadar Event Subscription in the installation with a specified ID`n")
...
switch ($input){
    '1' {
        # Get all the QRadar event subscriptions in the installation
        'All QRadar Event Subscriptions:'
        Get-CAQRadarEventSubscriptions `
            -Connection $connection
    }
    '2' {
        # Get all QRadar event subscriptions in the installation that are enabled
        'All Enabled QRadar Event Subscriptions:'
        Get-CAQRadarEventSubscriptions `
            -Connection $connection `
            | ? { $_.Enabled -eq $true }
    }
    '3' {
        # Get a QRadar event subscription in the installation with the specified ID
        $subscriptionId = Read-Host 'Specify the Subscription ID of an existing QRadar Event Subscription'

        'QRadar Event Subscription with ID ' + $subscriptionId + ":"
        Get-CAQRadarEventSubscriptions `
            -Connection $connection `
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD8ft9sKlLXSm83
# RxbT+2hlPVh/s6/VcjASXJLZETRrmKCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCBY8N8+Iz6E1CjgmuyXHHkq0AiLPI2+8bEeoSVhA/V4nzANBgkqhkiG
# 9w0BAQEFAASCAQCB3Zsy2no2tZVTrqUlWp3v9LuS0GcDImuXgoYWlZl4nG+Mx9jZ
# vTR+SUWP2ms8dTtVXt/q+8hm54i3xsVDCa1AjUSuaqxKJfT8T2CJqq9EpTHhu0bn
# QnQuN7ytOY9y/bwLshYD8ITeeytumRXQ3btRAqe0teVgDdChD5tKjnQS0bTyLJwQ
# H2ysynJEMaWeihHCAvSr0XvG/XCIbgFuCmCticcojKQGiVabq33e0Kj9Hkq+jZBb
# 0ZcHnZ4j9t3nbvUOE4dNnUC4e5bQvL/z/yaU1PVrT/qbFlXq0Bf1trHh49Ornfs1
# uGQCa9KS9qd6HKRqX6nQMD9BodYD22ESwAvfoYIXQDCCFzwGCisGAQQBgjcDAwEx
...
# BQAEIG4qvH68XhaUeheyiIaHdAHviChKBdrZjvBNY9GcxiV/AhEA4qxwB+3fVg+e
# kw6kHe2pHhgPMjAyMzExMTMxNTM1MzFaoIITCTCCBsIwggSqoAMCAQICEAVEr/OU
...
# DQEJBTEPFw0yMzExMTMxNTM1MzFaMCsGCyqGSIb3DQEJEAIMMRwwGjAYMBYEFGbw
# KzLCwskPgl3OqorJxk8ZnM9AMC8GCSqGSIb3DQEJBDEiBCDGgNU+cqyDSXeqnMfc
# a+gY6BfV/Lm8aQbHl3HkZRCihjA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDS9uRt
# 7XQizNHUQFdoQTZvgoraVZquMxavTRqa1Ax4KDANBgkqhkiG9w0BAQEFAASCAgCa
# 2lRw4lyN/U5VwtGqr0KYoHo2dZi5vciVuI5TdYbAyb64eZeKehZ9+zC0+QhXo3qZ
# Y8PMkdOLoVHj3QWpr2pcrzhA8FYQQuszeiZuInaWSaYUWA4ZbbQ5WBtmu8olTZya
# JAQUZU66DXRkF0QMtKLSbjMbPfW5BdKLcSSUEsGYfUJsCwA5ku8m/5+ZeMhC06gv
# u6d9Bv73lN9C15wu92Z9ftVuHRnO/s16rjAYbDrfG/O53Pn7Gtc+sXf/O+RqEqbj
# vSnB7Krc8hgCLdXj5dBhSuQWdogfJnTBL5WC1NfQmZQYDqEC5ku1mvjyPR0fVCP6
# XHpCKluuKX45/JK1jYBVnoAqGQMXtfHUIPLY2/Mbu+x1he6i9rErThSzM62FC4F7
# IlHgy5/k0ZyqJ4L/Ejwj9rYeQCwzyHOGke5dPrtYELB2URSgALwKK8l/qP6deOuE
# MbX1+1pvoRw7fYdHDSg2HDmjVnBvXsUhxBAFsOSfHHuiMzRSj47Ey+DOZe13ku4U
# sZcAV24U5r/oOS9SJZr5itPtpRkxR2uwjeiNSLPzsQ2jrFe2fX/vprfM+R7hbBA9
# IO7Z+jIZRvy4Yeyad2LxcqNTr8zEksQjFdZF62Asd7oZnrCai9Vl0CgrLLHsur/f
# 6ZN61YqRtqdqPHszj4vkc4DesBmaWSPYnBPVn4uFLg==

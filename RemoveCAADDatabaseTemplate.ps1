...
# Gather list of existing AD Database Templates
$removeTemplate = Get-CAADDatabaseTemplates -Connection $connection
...
do {
    ...
    "     1. Delete an existing AD Database Templates`n" +
    "     2. Delete all disabled existing AD Database Templates`n" + 
    "     3. Delete all existing AD Database Templates`n")
...
switch ($input){
    '1' {
        # Remove a AD Database Templates from the installation
            write-host "Here is a list of the following AD Datatbase Template IDs" 
            foreach($r in $removeTemplate)
                {
                    write-host $r.id
                }
            $deletetemplate = Read-Host 'Specify the ID of the AD Database Templates to delete'
    ...
    '2' {
        'Warning: You are about to delete all AD Database Templates in the installation with their Enabled property set to false.'
        $verifyIntent = Read-Host 'Type "yes" to continue or "no" to exit'

        if ($verifyIntent.ToLower() -eq "yes"){
            # Remove disabled AD Database Templates from the installation
            Get-CAADDatabaseTemplates `
                -Connection $connection `
                | ? { $_.disabled -eq $false } `
                | Remove-CAADDatabaseTemplate `
    ...
    '3' {
        'Warning: You are about to delete all AD Database Templates in the installation.'
        $verifyIntent = Read-Host 'Type "yes" to continue or "no" to exit'
        
        if ($verifyIntent.ToLower() -eq "yes"){
            # Remove all AD Database templates from the installation
            Get-CAADDatabaseTemplates `
                -Connection $connection `
                | Remove-CAADDatabaseTemplate `
                    -Connection $connection
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD/ZIibO35B+TiO
# aktned89Egi9aopY77i8qhEFt/v9C6CCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCDX4GuiGq/FNKPqu+0j386gb83e1hE2jXeYS/3B+vjt2zANBgkqhkiG
# 9w0BAQEFAASCAQAzWaodQfcpmmx6u/B8guMLFZMEcNtQt7YUaY1q7sCELak27x+9
# TObnnIXqrL7Vqq1A4GUs9G1Ffc+ZH6AICk4QFCUhHp49og+mRo4OiDPSsYmuJciC
# PnGImW+56eynJNZDA8DzR6UqtFxwQ8nMyLrVkcFvM1FMDPhloi6CrhJgvGmMETxV
# 1MEpLYAojEfQUYuffwBfv9kOhrdcb/GiR4n/qHGEqMGfey1hdjzd8VKcRdjyWzb/
# LjLyov4A0vmENryrNUasRolZVKn2cjcE/pqrabl7ag9FbO0LxZZHlAdtggz30VgR
# dBfJkpe3CT9zO9C2qccN7Bsi/ufplCwED36LoYIXQDCCFzwGCisGAQQBgjcDAwEx
...
# BQAEIMg5STQCt3ThSb9joJ2OlZpyG4qP35o4Ky+OfOGejYhjAhEAsk3MDfpCpIQC
# MofTqyUkHhgPMjAyMzExMTMxNTM1NDNaoIITCTCCBsIwggSqoAMCAQICEAVEr/OU
...
# DQEJBTEPFw0yMzExMTMxNTM1NDNaMCsGCyqGSIb3DQEJEAIMMRwwGjAYMBYEFGbw
# KzLCwskPgl3OqorJxk8ZnM9AMC8GCSqGSIb3DQEJBDEiBCAJJ9edhuSibISRpAQs
# gBbGxri4c9NRuUHVewsO+lOIWDA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDS9uRt
# 7XQizNHUQFdoQTZvgoraVZquMxavTRqa1Ax4KDANBgkqhkiG9w0BAQEFAASCAgB4
# PTgZbKL0Z6VLz+ctQeXHycfNXKOZhJpKSv6VsWRvDCRgLpX6dAg7KeuM1kbo6NtZ
# bXyNuTptvm+7bE9aX2GE9qythFpWB0ZOlz1Bp07dQd/viExfJeN0VCUW4QH5y9er
# HFhTt1/7Nx+3NiR/vg0XxxhTXl5HCcFvRMXD1X2X74Y2R3fYeXhAvOtpvANNe/Rq
# QCvjxDlro6IqXSbj8qNM+jp2aQpA7j4ik0oQRYWjnj24pzsC3W/MbzUOLm9aFlCI
# 02Ye2R+48pkGqohY+l45qG9fOL5hsnopUh8sRqu4dCVx7dOXX8Lb/Ib+QI6FW7xA
# oOi00pgAeXHyo1EQnGcxR1pEGIBqhExS8pKRRPpaWOFsZvxFO+bsx2bwTurKZFK6
# PO4dSybZO5QRV2tNiNHYLT7JjfBEiyM3v3rP0efL0F63qr9Li3f88f8yQEkLMm55
# Yv8AcnQR1P4RB8X2C0LJIwUbz80XFwrNOocDevR39odV8H+vhbEhwUY8eJrGZZlW
# tTCtyettw+fcYyooLMDbbwH5CKhpXk7QThsUSsLdBLyIvmvkjYe40t662IduDBvN
# BM/NFaJwCN585XzeUvvGgt1aHTS44et2/2wkgxp6e7MkUTlBEoGT/3K3DLwPUZd2
# WtxEjOAgk5+SEjAsxb943eBs9Gv0vmsRgSTWwXIsjA==

...
$validInputs = @('1', '2', '3')
do {
    $input = Read-Host ("Specify the number corresponding to what you want returned`n" +
    "     1. Get all Microsoft Sentinel Event Subscriptions in the installation`n" +
    "     2. Get all enabled Microsoft Sentinel Event Subscriptions in the installation`n" +
    "     3. Get an Microsoft Sentinel Event Subscription in the installation with a specified ID`n")
...
switch ($input){
    '1' {
        # Get all the Sentinel event subscriptions in the installation
        'All Microsoft Sentinel Event Subscriptions:'
        Get-CASentinelEventSubscriptions `
            -Connection $connection
    }
    '2' {
        # Get all Sentinel event subscriptions in the installation that are enabled
        'All Enabled Microsoft Sentinel Event Subscriptions:'
        Get-CASentinelEventSubscriptions `
            -Connection $connection `
            | ? { $_.Enabled -eq $true }
    }
    '3' {
        # Get an Sentinel event subscription in the installation with the specified ID
        $subscriptionId = Read-Host 'Specify the Subscription ID of an existing Microsoft Sentinel Event Subscription'

        'Sentinel Event Subscription with ID ' + $subscriptionId + ":"
        Get-CASentinelEventSubscriptions `
            -Connection $connection `
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBh/2hZrI1YWxbB
# fgINE0RF9RtoUn7cGaV/S2/fUoE7+6CCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCAvOped/9yydQTu2F+VpmYSFhZaewGcJs4aXk6VjF7hADANBgkqhkiG
# 9w0BAQEFAASCAQBfsR1PWrE9Y6w5rjoR1D1pwwL2j1cDR+GJK8TqBGEvtocG9R53
# XAtxQNqvBDs4/Lg5LCAIs7TPeVYk10MJ+J+yABg9jzk7YU9gHqZmDcE48yS0eKEI
# PxjUnG8Dj+fgEs1LwlK5jrxke0i6q96VQ0d3n3fN/24vYFld+V0MSaknuPgaAXxv
# LIa/2u9bnw44oxfGCulBjBJVPmIL64dbVjrOBYeJxh0DSE10bvLvAmSg8t+DUPj0
# DpOyqW8SObQVQty9cHT3E5lxoV9DuKQJphDLbHElmEq72/clw3Gt3/nPdtt5CqW1
# tSd+A4U3cuHcCHi1JK768dfvXy4638CnnSJXoYIXPzCCFzsGCisGAQQBgjcDAwEx
...
# BQAEIFSVvh/lq3hgXSoILVu+0wSIBQNyrhv9zdzkq5nRbTSOAhBWvCOlWZ2Tzn76
# GqSPz5l2GA8yMDIzMTExMzE1MzUzMVqgghMJMIIGwjCCBKqgAwIBAgIQBUSv85Sd
...
# AQkFMQ8XDTIzMTExMzE1MzUzMVowKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQUZvAr
# MsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEIBS3jMoe1Iq2zef6sWFQ
# SANG6R1zGV8QYeCS/Dk6hqa9MDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3t
# dCLM0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICAB5X
# dItFaLUZlb0+3jNhKLzBy8nOMu5g1pnPj+jhcP8wMnYwUY+r+afl0LzjA+DU+rDj
# qufTsTL0r6ohNgvOwHw8r8e9pK18Bl35GoB5N8plELTNdwy5QMvgTR1kdJNP6ShE
# Z/gCUa3E79qKIKIVEv6SuUVhz25wXJZFyEGkb5VmIHHPEO2jI7Tb87w3PYV4XQXx
# 3P3py98arYZD73VSf4o2IhcpUPzEOBDRfvI72QUfl5+jq5B9kHhBWp+CTaMuixx3
# 56IR8bTTFN6BfuTRk1cogEopuqD7tY1Z19YECKtnM+1Knj9O2ctw9QhkwxBUOPF6
# pJwa6yD4qVEREBqfHUlE89Gvt/pVwsg8c4gpMAxXCEbCzcG1755ToUxXoKOaFpUQ
# wC9NwCkJkdVNlrnCzclN8RXmDP5k/E1q1EWkneC20PmjxEuT/bDJ1D3HQYbasyPn
# aFaFLn8GOnYKhiuiv/TNsa9iJUc9APCVr5nyrjnHrxC+hI94txgpDCSVXaNgKJZT
# A9F5mCjBJCvqxZ/vS5j4skIB4W0ZoWa2SvQUloZe0yCQxDEVQI99d/mLhrU+7ZbV
# V+zsMcTfHV6uXBct8BsTwe+BrbTg+ynmguAfFOOaQew2O0i8YW7PjLeZqtFDzVKd
# YmR+srAdTzsxeUiwqufkN3C7JDDyAhteEwVnfAS3

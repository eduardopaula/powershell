...
# Prompt user for agent host to use in Office 365  auditing template
$agentFQDN = Read-Host -Prompt 'Enter the FQDN of the agent host for the Office 365 template'
...
# Create the new Office 365 Exchange Online auditing template with historical event collection. 
# A browser window will be spawned to accept Azure logon credentials and (if not previously done)
#  consent to the Quest Change Auditor Configuration web application requested permissions.
# Note that template is created in the disabled state, pending addition of mailboxes
Write-Host "Creating Office 365 template, this may take a minute or two..."
$template = New-CAO365Template -Connection $connection  -Tenant $tenant -CreateWebApp -AgentInfo $specifiedAgent `
    -Disabled $true -HistoricalEventCollectionDays 7 `
    -GenerateCertificate -EnableExchangeOnline $true -AuditOrganization $true -AuditAdministration $true

# get the user-specified mailbox from the tenant
$mailboxList = @()
$mailboxesComplete = $false
while (-not $mailboxesComplete)
{
    $filter = Read-Host -Prompt 'Enter the full or partial display name of a mailbox to be audited. When done, press Enter'
    if ($filter -eq "")
    {
        $mailboxesComplete = $true
    }
    else
    {
        $mailbox = Get-CAO365ExchangeMailboxes -Connection $connection -Tenant $tenant -SearchText $filter -First 1
        if ($null -eq $mailbox)
        {
            Write-Host "Mailbox '"$mailbox"' not found."
        }
        else
        {
            $confirm = Read-Host "Add " $mailbox.DisplayName "  at '"$mailbox.EmailAddress"' the mailbox you want to add (yes/NO)?"
            if ($confirm.ToLower() -eq "yes")
            {
                $mailboxList += $mailbox
                Write-Host $mailboxList
            }
        }
    }
}

# If any mailboxes are selected, add them to the template
if ($mailboxList.Length -gt 0)
{
    Add-CAO365ExchangeTemplateMailboxes -Connection $connection -Template $template -Mailboxes $mailboxList -AuditOwnerEvents
}

# Enable the o365 template
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCADYbytFKEcTPCr
# U9DwuHqWBBIkt7IsnnrWe1iay9qqH6CCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCB9PdmpVPA/IvIltRYW/mfHTjuhbqnRk2R8LcRVKAUsuDANBgkqhkiG
# 9w0BAQEFAASCAQAo8vdWEc6OxY0HPbEar0VBIuMbAcfhUa9POYS/ODB+R3Zznvty
# GTNU3BTbOssDZaIjDYikU3H6MXG7LXVaF0Xf5KdHQ0hCW/NiO6sVNq60VY6weJDu
# EQVRK6mj1ph5V8qtYoCSekIGSPuEYo5+uZuU5HKk20IbF28hVVnR3Sc8q6koRqgD
# mLGMA03h/XK/4nWlmjaGyeXV/2iyCgh+f4CBgby4uCnR4SwnNHy9eV7C1B8lRy14
# S4QiKxP92C9YOU23mUCZxIsVelURFYsxZqa+4wqGAlxgKqspsYyyclxvB/q05yRR
# Acxk6JrbAh0nkuoefHnXr2AQHFIx1BVaXveloYIXPzCCFzsGCisGAQQBgjcDAwEx
...
# BQAEIBsaTvaTsYORQi61O98CXJPa9/dU0iEM3oacU4LAAZH4AhAHYPHhqlXcREsz
# U0hvesEPGA8yMDIzMTExMzE1MzQwMFqgghMJMIIGwjCCBKqgAwIBAgIQBUSv85Sd
...
# AQkFMQ8XDTIzMTExMzE1MzQwMFowKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQUZvAr
# MsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEIF7N5iGFzVk0sjJJusJW
# 5ESEWnH20+1oRiAy8CRz8PSgMDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3t
# dCLM0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICAEnj
# 4a3ChKGuywdQvgDYp/5op1jJH6idIgVRf1Wy3wDPlxeFL/RkHjwULYOpvJ1OaEye
# fbawUk7Ub7BcCOmpUbbvzZCNyHG3l78qRGqeojWNaEeMuYYRg5roe5Idpbl1StYO
# LB7TwTVThEJk085ytMf+iYNY2MK8E0aglIr4fqg3Rquu22EGwmPaj6g7DLCFbYAy
# CghOSWm4D+FkkgrqOZHTTcg81/MyC6vPECv2X1C/2V1FvM+QpLwGweSYTB1Tx8RP
# sxts2DXLORDH6RA9PjL2Wli4iqwSpf76Ccc3+r49WQdwMtMJfkEKdxt4tHWR3W1o
# hfFNlQ9PKepXdZPNIksX5L8r7heLPCUOYmldDjeuOST+hbnMJblkf1cSDUpqtBfj
# 0bPt4AeXUbeOck9yH1Pd2dYkwb9qgvJaH0CTBNslGG3auIOe/0Np0wHS0N3fEJVz
# mazQ6pNLOQGqzAySZxh9zd2SnlsJ2GEMexPgwvF/W+8N0TamtdfAUcF7FkPj8AK5
# 034imB2Xj4h46bIC3q1ZohsaKxEupJxL8ZlTzEs2griLdpTTY+gCCIMFrwDtqDIq
# iFDtzN1pOly+pmZeF+ZkNGhgTz25zkGqFMIcgh062LRNRQJU2Mux0kV6jNsT0XYi
# kDuG4Llc296K3laGQIRgdcID4WxyGG0GpLnIMbZc

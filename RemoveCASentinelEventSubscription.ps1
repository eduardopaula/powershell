...
do {
    ...
    "     1. Delete an existing Microsoft Sentinel Event Subscription`n" +
    "     2. Delete all disabled existing Microsoft Sentinel Subscriptions`n" + 
    "     3. Delete all existing Microsoft Sentinel Event Subscriptions`n")
...
switch ($input){
    '1' {
        # Remove a Sentinel Event Subscription from the installation
        $subscriptionId = Read-Host 'Specify the ID of the Microsoft Sentinel Event Subscription to delete'
        Remove-CASentinelEventSubscription `
            -Connection $connection `
            -SubscriptionId $subscriptionId
    }
    '2' {
        'Warning: You are about to delete all event subscriptions in the installation with their Enabled property set to false.'
        $verifyIntent = Read-Host 'Type "yes" to continue or "no" to exit'

        if ($verifyIntent.ToLower() -eq "yes"){
            # Remove disabled Sentinel Event Subscriptions from the installation
    ...
    '3' {
        'Warning: You are about to delete all event subscriptions in the installation.'
        $verifyIntent = Read-Host 'Type "yes" to continue or "no" to exit'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA5Mhf8e6th9PmS
# 6iymSC4pJnTDFkyIWonFI0a48SH0QaCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCDchmMdFGjiTGhSgAEsXKxkzAmUwQP/Byy4DdspNlrZWzANBgkqhkiG
# 9w0BAQEFAASCAQBqc5PDO2CzNPB4WcUSd2nro4yg60xktFktzpQQoqrPhVZi7iuw
# LJJzCUjO5GMk9zQpJUlhDzfIow9YGz/dJDkM6ozz/rZ4mi1ZcHMqPfY6eHQ1Tv7h
# baPvPbcUL3V1qDtMnMgfh0B+KxOJXVEQ1TpyRH/urNO7tFHqkof5Q3MADV5KhsTE
# sjB6IoF9AF29mIwnVxl40fIoutGCVemfi+2maIEKrBhhD+EPCt2HVo/olKUwVFNm
# 0fRgmWz7a/3/B4SI3ihRzb1oCoVBjgcF3guWSjNrK80LQclgKgLEKIqeSctoc+Q1
# 7C9Qz8oi4dthoB5ZMLWx1uODEBDPZlWjZwa4oYIXPzCCFzsGCisGAQQBgjcDAwEx
...
# BQAEIFxFY5V+BKSbI1nyRDET/XSE3+LMKoAbNprF7aeJLvXIAhBOYGbndJ0qw8Py
# 0cjl5G7oGA8yMDIzMTExMzE1MzU0NlqgghMJMIIGwjCCBKqgAwIBAgIQBUSv85Sd
...
# AQkFMQ8XDTIzMTExMzE1MzU0NlowKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQUZvAr
# MsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEIJYfZ8Db95HuXRnmKRbB
# 7lnG7GjW7wg34Sd30C3NysCcMDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3t
# dCLM0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICACdL
# WwLEN4TDG5pGcAD9CZDoPIDCaYMgJCrE/fL4231ASf6P5JCNktBZQdH9bDrPN/VA
# HI8v+2bBRKbWQc+ZZWgDLbCD3b2IBgJZ0PaifCX0KBKzK2RN434dUWDQZwycbLf0
# 8RsF8qCD45luKHsCk+pSG0RCKZ5Zoh6i1iEl/yzF1utDY+0P3ataoAPVkj6GnuN9
# HQAPnQLf5hZKX/PlKJ/pZthE/zI9hFDkrWXma9G8zHqIbhYpb4HuFzHfdGc6spCS
# fRYK0KMx8xnDVpsSydI+p7JlrMqNyqvItC1B6il2AVQOQKuP1DjiTum1nbvUQba4
# pJsTdIX9F96uS1onGMBTaeBcpLY/ryHdza/EiG99Pu4+6YiAFdU+s/0AZXrsQyXz
# iEFVNGgWSwNSE/HIkkcHTXZclWEx6rVKnbt/sBawMLHa8RsRbWk2sucKbPtmmtYm
# BUD/2uKQjpe9lr4Hz2f3c0AalHej6JTMsFrFiNtl9LvzvRJ4W3Wy5P23W1XDJGZt
# SEya5zeGnjUiuyxMGKUmmxtoc7FteYew4k2U5OJV6n+nDbs54oUQjvZUL8mW7hVu
# +hL1s8dcQGbQJE2N0K260vPz0PpNTHt6Myu9MZhWBQkHAhSOhJ07l8YuiszqcVuv
# JgZfst9MeRzl+AlIcc93ifrTiCmubjQfOLMf1tZ/

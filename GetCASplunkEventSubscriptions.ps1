...
do {
    ...
    "     1. Get all Splunk Event Subscriptions in the installation`n" +
    "     2. Get all enabled Splunk Event Subscriptions in the installation`n" +
    "     3. Get a Splunk Event Subscription in the installation with a specified ID`n")
...
switch ($input){
    '1' {
        # Get all the Splunk event subscriptions in the installation
        'All Splunk Event Subscriptions:'
        Get-CASplunkEventSubscriptions `
            -Connection $connection
    }
    '2' {
        # Get all Splunk event subscriptions in the installation that are enabled
        'All Enabled Splunk Event Subscriptions:'
        Get-CASplunkEventSubscriptions `
            -Connection $connection `
            | ? { $_.Enabled -eq $true }
    }
    '3' {
        # Get a Splunk event subscription in the installation with the specified ID
        $subscriptionId = Read-Host 'Specify the Subscription ID of an existing Splunk Event Subscription'

        'Splunk Event Subscription with ID ' + $subscriptionId + ":"
        Get-CASplunkEventSubscriptions `
            -Connection $connection `
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCVGbaEWLvXtujP
# ZDl2laeFSrYgVWR0EmmQtsMCknhX/qCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCAN0IgApcOioY7MfkFr2MVdqvYs4/PbEfM/w0aHbKWXLjANBgkqhkiG
# 9w0BAQEFAASCAQCxG4lmWwbEGKaKQRJQGww/lV3P8oc5bnbhbmYoE/5v3XQEGOt5
# PkR3CeuDrs8HihQKtKjN0XXzcUT+N08RrApkTJqxip/m1WHHtg4Ex1XgB7seCx2+
# 722ncqEfvRPgWxSFwuu/jSCdW0viU8bEMzhGtg3oS+X3u8twqopZ3MQFnMxOKVyI
# O9X6O1eIylaNZiYKya6dPlBvOfSA3+Y8w9ygtd4sssnec0apkUZlC9xGZS63FfUv
# I6Y5JGze/EOLH5E/WDeBl3e1FPheIxJkL7OPMukxyLe0q1gCEQJ4LQrZoYqjH2UO
# GJJUEzxkBV2oWwB5w+PV6R/I0zsyKdOo/RUNoYIXPzCCFzsGCisGAQQBgjcDAwEx
...
# BQAEIIhuljg1ydI3YhtGZqkuqp+Rnj/BI9s4NweRsaG7sZR7AhA9/o7+G2zoH9J2
# fIfel0Y5GA8yMDIzMTExMzE1MzUzMlqgghMJMIIGwjCCBKqgAwIBAgIQBUSv85Sd
...
# AQkFMQ8XDTIzMTExMzE1MzUzMlowKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQUZvAr
# MsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEIJVsXOVdSSVnK6xt1BGk
# j2Pe/n517H2EXfq3nJtk9FqNMDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3t
# dCLM0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICAILx
# op7AKq18CYsXTR9ASkzNC3ujAw7ZKkPGUoyG8tOrVhC9/5QoNMsF0xkqWRVrObdY
# oIfPCp/w19C2Y8P2op7XS82m5z+eAeB3bfvky2+5ha5iP26YzPbFaepxRoLPnYmo
# kqxDqlOGOBlHSJDt54KvJv6xuqaZTyvr9rnpvSlg3TodLLn5trGWF/ZddOOfbc5M
# vaLoOse9+Bi/xoL8VrK9/L3J+D1mp6Ws20URRL8q0Zg6u6bJ0gGxzzBFOsahSKxo
# 29sX37e0Ulk2VDieX2f7nYG979IvrA499GbWn6uYJpXFY+HkkQw8PKyq+pZzDkzd
# lqRFGSh4J6r61bBZgvRaTyM3kBkqzmfK8otMjoKmBxewtEPhYOViLvjefR4ek+kQ
# MITbggPVXAeRT9Bjjsyb9HrLDTvbBf1n1T4JZsNmaKVaRY4uI1p8WpzxFQ/FSgoz
# sr9NfS46N4Euwgw7MGiUFWU/hH1rVlQnOjg+VNWjnG0FmuSluMMLC3Fwy+4Ie4k7
# 9NU7hhdb2x4umsCC+PEm97ryYNTjKRg+XXv0NBBti+llpI/13tpHlqlKMBOBAbnu
# wwFl3dyZi9qGUoGG20mzhGC2fJAvu1rn2tQ8ODzDjoV9VNVg/PXyg/+EnFpqnJQO
# CRFBNr8uqPt7QKit2vwvIFIVZisfgCygEo5JmslZ

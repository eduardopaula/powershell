...
# Prompt for the webhook Notification URL
$notificationUrl = Read-Host 'Specify the webhook event notification URL'

# Prompt for the webhook Heartbeat URL
$heartbeatUrl = Read-Host 'Specify the webhook heartbeat URL (optional)'

# Prompt for a Notification interval
[uint32] $notificationInterval = Read-Host 'Specify a notification interval, in milliseconds'

# Get the starting time
$startTime = Read-Host 'Enter start time (or skip for current time)'
...
if (-not [string]::IsNullOrWhiteSpace($heartbeatUrl))
{
    New-CAEventWebhookSubscription -Connection $connection -NotificationUrl $notificationUrl -Subsystems $selectedSubsystems -HeartbeatUrl $heartbeatUrl -NotificationInterval $notificationInterval -StartTime $dtStartTime
}
else
{
    New-CAEventWebhookSubscription -Connection $connection -NotificationUrl $notificationUrl -Subsystems $selectedSubsystems -NotificationInterval $notificationInterval -StartTime $dtStartTime
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAag9LSEqp9i+zZ
# PVK8Mqy8vO8yjxRIQJ2v4OWJKbWLqqCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCDybNFD/ewW7cphHH3j2FZ0VoCbOE/V+PLfR8DJDJrRJjANBgkqhkiG
# 9w0BAQEFAASCAQBh6AAlKq2f682StL5UviRaOj5kb3iw/J76YB9boxrIqyUcXemm
# M6IzCFqy4NZmzRcyACRBzhc6Q84Xl11KmQz2zbZXj8Pg+E7aw0DQ6prbAYrKkyqc
# rxL70QrCvU8/Juc7Suxf0CHp8qOHhvE/fFa/jSq87pbLS9YPXwo64rtdHZEwGH0j
# B5Ts7+e4ajpm9/M0pc3zIkyNRWAF3CpPFiCQMeb9QfCWD/3eP0FWokFzTuACYRSO
# dF0Ev6z1bnUbDo5FwiJtWVmAp09A8bIcSFlV+r6ypuMl715w63zfoAc2ii3F65jV
# OmCbH8D6surt9VIfhLIGi8EsaEhHoNktUlnBoYIXQDCCFzwGCisGAQQBgjcDAwEx
...
# BQAEIIOa2VydbkhWalxW1ypzfIfHuAybDfdX8QfxElP6oBSeAhEAjmSD7l7f8In6
# ziPVBTIFbhgPMjAyMzExMTMxNTMzNTZaoIITCTCCBsIwggSqoAMCAQICEAVEr/OU
...
# DQEJBTEPFw0yMzExMTMxNTMzNTZaMCsGCyqGSIb3DQEJEAIMMRwwGjAYMBYEFGbw
# KzLCwskPgl3OqorJxk8ZnM9AMC8GCSqGSIb3DQEJBDEiBCA1k21zamUqdx0pXVl1
# rI/4Zei9A0m7/I7Tk0YyA065aDA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDS9uRt
# 7XQizNHUQFdoQTZvgoraVZquMxavTRqa1Ax4KDANBgkqhkiG9w0BAQEFAASCAgAg
# YRqlqRs776ZJs4N9Xl8zoA9z2Zje7jLYR8gBxm0hcmF8ssBPzOhAqDEqPTuwEttr
# u/3C9ZaKh5BP6vN+R18px9T9bqiqi3SoGi/UfrHxmdlCrqUQsw5a3DX3UWqx2l9c
# h0EY7iMKGqyJWNjf/ROVHHBd/00ka70XLQNhQH/XuFpBrS1A3tEUomVMzDAGIE7G
# AAb33mmV9qjrzJpuF7hnwTFDWvC158ME0LU3HKgMwdSXMn4xtQWAtORoVhxxNLtf
# IMZu0WEuF+cnTZISMf4QSXnP0MzY6N7rtGC9sPpf636C+MT15b5lBOUsCh+CG1ea
# 6FCK34QSOdIeEQYYeY1XuhuIgOgqtm7w11m2mqzumteDNZRWElECXuUIbnlnAY+d
# 0eb35FpidFzerCFiU2hLbDKMyUHvpwk58kOZH9NbkGDC5X6IuCm5Ms/th4jf0N+S
# x/7yErNfo2OBgJ8XY0BAOHOOVffoBVy9gPc2tSqecgRQfe1Fc/TqP2x+sZPY1P58
# NUiEIQvtf+GnDLIcWRLr1yQDkWmdSGs7tbMk4bEx7qHCVEXiw8tueMkoiriF3EuO
# JAbDRGpn2jGucLi3M4pb94aLEARBmm9D3ONcnD2qnobqxrh11AN7oTsq/670mqzP
# eg9ZGf8cEo4MQIPQfqoh/6JcGCVlUuOk//PW/vnqXg==

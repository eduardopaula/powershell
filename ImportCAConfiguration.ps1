...
# Getpath of the saved CA Configurations file
...
# Import CA Configuration from the exported CAConfig
[xml]$xmld = Get-Content $fileName

$response = (Read-Host 'Include Application User Interface configuration settings? [yes/NO]').ToUpper()
if ($response -eq "" -OR $response -eq "NO")
{
	$includeUserInterfaceConfig = $false
}
else
{
	if ($response -eq "YES")
	{
		$includeUserInterfaceConfig = $true
	}
	else
	{
...
$response = (Read-Host 'Include Coordinator configuration? [yes/NO]').ToUpper()
if ($response -eq "" -OR $response -eq "NO")
{
	$includeCoordinatorConfig = $false
}
else 
{
	if ($response -eq "YES")
	{
		$includeCoordinatorConfig = $true
	}
	else
	{
		throw "ERROR: invalid response."
...
# In non-interactive environments where using one or both switch parameters, 
# append -Force parameter to suppress their warning y/n console queries
Import-CAConfigurations `
	-Connection $connection `
	-ConfigurationSettings $xmld `
	-IncludeAppUserInterfaceConfig:$includeUserInterfaceConfig `
	-IncludeCoordinatorConfig:$includeCoordinatorConfig
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDSKmyIXuyT8xTn
# IHe7ZiXAlmHv/O4sCcT38SZTBxQ0fKCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCDZNnM7lU+mGglDvz6IcOJaXYyLBSqq88rm+RraWrZpRDANBgkqhkiG
# 9w0BAQEFAASCAQASPVvwk8mrYWRIqhstVnsq9+kMqvq4rKOhpATZdb7c+MHwE5YI
# PX5xrPQ1D5jsmwgss/26QOJXTY5FpnyGs8ii6JCtVO3H89ExUbIuQ+cnUJIdoLOB
# 8FlcEcW0TBvzs5KHcrbRdYLF1yjCBZyVUOf9g93foBI0pA+iV8OuA/6eAsgyGpLj
# O9fuJgeYYWb0ju8n0Cv0SqqzHUyNQD5yDGHNbr+SU9EFadXPZQiDXgAMI9EQrb81
# P+O0T8VsOhUyUVvUfuOJtGYTuYJUpqk17WubCPfSVh6Jkda/txZugcPmZ2e2LhsC
# 73s7GowXXksRaKry4qZ76myp4Q0mu258vMCcoYIXQDCCFzwGCisGAQQBgjcDAwEx
...
# BQAEIOzXoxUzqcckZFwgvyi92X08OtIV3z4P23qtAYaDRcgXAhEA3YlvkH8XE+1k
# 1TF3/CFNWhgPMjAyMzExMTMxNTM1MzVaoIITCTCCBsIwggSqoAMCAQICEAVEr/OU
...
# DQEJBTEPFw0yMzExMTMxNTM1MzVaMCsGCyqGSIb3DQEJEAIMMRwwGjAYMBYEFGbw
# KzLCwskPgl3OqorJxk8ZnM9AMC8GCSqGSIb3DQEJBDEiBCByU+tbX7VI4lwDGSmK
# hGvGexvpkM72ZrK7kfFuccSgwzA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDS9uRt
# 7XQizNHUQFdoQTZvgoraVZquMxavTRqa1Ax4KDANBgkqhkiG9w0BAQEFAASCAgCd
# VuKLKf0MJZe+8/SIqXKFJJT1/kfiOgw/TlsgOgCi9LuacF1PWV2/IsQ0E63drzPm
# Vh5SsSM2Eq+3BoQtSJwZfHijtsKIIufXRA//BY1YQjZZ8mumxaj1JaN+2F53/4ob
# P/gMesyiVF6m5mgK+XZ086096fIepIEZ5oT9+CGbnTF8ZX5cUT9vQX+2K2Uqeqi0
# ut1NibGQQbI3Ta63Vr2P/sk7ocfUDRzEL1ORLwvX4H7lupRM8rBwp7xXWFlf04IA
# XKBiABjeDmmt//1Es5qEDAlmApZ/LCVbI6KLTuM/BthkRR/qHvUV3Q3rVSHQd8/c
# WaULR5yfzAuJ81cOFdh1PRv1uYMxHDGZg8+X8aW6+wi0J2KLfUW5W3FdcshIyVuu
# iIUfF8FTtpxxXigfVzDKagwC8iUSZ27mFxYrocpumf3yF3XaeDH4ETD+O3nN5h1u
# eENqtZxp5njiZQRHT9aRsd1Rh2UUymaMwbamhYwBuKaaHApVJFXj1RoamRBcG6ui
# Q2q8OlIsrT5tUAHcFowyxkCpdyW1gzj1jKQrg5b6O7S/GrG0UFSAyxzeg1m1OYgJ
# VP4uvYPEW4aPao3wsXBHr7JRUVMzCebozRfM9Bu4RpJrq5pU44WPjBoYJzBJrd2Z
# PF8yesHgDjTNkrJwY1uicfiND9ysqb5gItoRYTsIlA==

...
# Prompt user for agent to audit the Extended events. Leave this empty if agent is residing on the SQL server machine.
$specifiedAgent = $null
do
{
	$agentFQDN = Read-Host -Prompt 'Enter the FQDN of the agent to audit the Extended Events. Leave empty if the agent is installed on the SQL Server machine.'
	$specifiedAgent = $agents | Where-Object {$_.agentFQDN -eq $agentFQDN}

	if (($null -eq $specifiedAgent) -and ($agentFQDN -ne ''))
	{
		Write-Error "Please specify the FQDN of an existing agent!"
	}
}
until (($agentFQDN -eq '') -or ($null -ne $specifiedAgent))

$sqlServerName = Read-Host 'Specify SQL Server Instance Name'

# Specify either windows user account name (eg: domain\user_name) or SQL Server login account name
$sqlCredential = Get-Credential -Message 'Enter credentials for connecting to the SQL Server Instance'

# Get all SQL Extended events classes for auditing configuration
$sqlExtendedEventClasses = Get-CASQLExtendedEventsInfo -Connection $connection -SQLServerName $sqlServerName -SQLServerLoginCredential $sqlCredential
if ($sqlExtendedEventClasses -eq $null)
{
    Return
}

# Get the SQL Extended event names of interest
$eventNames = @()
$eventPackages = @()
do 
{
	$input = (Read-Host "Please enter the SQL Extended event name. Empty to exit")
	if ($input -ne '')
	{
		$eventNames += $input
		$package = (Read-Host "Please enter the package in which the SQL Extended event name is defined. Leave empty for default value")
        $eventPackages += $package
	}
}
until ($input -eq '')

# Create all the SQL extended events objects of interest
$events = New-CASQLExtendedEventsObject -EventsInfo $sqlExtendedEventClasses -EventNames $eventNames -EventPackages $eventPackages

# Get all the filter information
$filters = @()
do 
{
	$action = (Read-Host "Please enter the SQL Extended event filter name. Empty to exit")
	if ($action -ne '')
	{
		$operator = (Read-Host "Please enter the SQL Extended event operator")
		$value = (Read-Host "Please enter the value")
		$filters += New-CASQLExtendedEventsFilter -EventsInfo $sqlExtendedEventClasses -FieldName $action -Operator $operator -Value $value -FilterType 'AND'
	}
}
until ($action -eq '')

# Get the template information.
$templateName = (Read-Host "Please enter the SQL Extended events template name")

[int32]$maxMemorySize = -1
while (($maxMemorySize -ne 0) -and ($maxMemorySize -lt 250))
{
	if ($maxMemorySize -ne -1)
	{
		Write-Error "Maximum memory size cannot be less than 250 MB!"
	}
    $maxMemorySize = (Read-Host "Please enter the SQL Extended Events maximum memory size in megabytes. 0 or empty for default (default and minimum value is 250).")
}

$disabledInput = (Read-Host "Please enter true if the template is disabled. Leave empty for default value of enabled.")
$disabled = $disabledInput -eq "true"

if ($specifiedAgent)
{
	# Create template
	$template = New-CASQLExtendedEventsTemplate -Connection $connection -AgentInfo $specifiedAgent -SQLServerName $sqlServerName -SQLServerLoginCredential $sqlCredential -Name $templateName -ExtendedEvents $events -Filters $filters -MaxMemorySize $maxMemorySize -Disabled $disabled
}
else
{
	# Create template
	$template = New-CASQLExtendedEventsTemplate -Connection $connection -SQLServerName $sqlServerName -SQLServerLoginCredential $sqlCredential -Name $templateName -ExtendedEvents $events -Filters $filters -MaxMemorySize $maxMemorySize -Disabled $disabled	
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCnNOMo4ucqN1mr
# 9dLEIyGCCR7qP7iuHmApdUxG4Cft4qCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCB0drJKvEOQfUsBQwj0o2rMom4Cntkw1cs5SVwykYUAVjANBgkqhkiG
# 9w0BAQEFAASCAQASh/k8HtDEdfk9lUWGTI7UbQcYzbxnqYuMd9ka197FVnEqhnks
# GAR6wZTK2lbCTFiLp1CAyaceezVk+rbA1eJSa4kLtBjQOoMxER6t+6N6goKJQZEQ
# 7OqaWhyagIYwJnoSWpIblaXLSFOz+WDOq6tRRbjMYlCl12/c9z69UJzfoIfhT+Wj
# vDt1N2zxbYFsTvLZaqoG1EaKXP7VgdMB4LE1t9BeLWPxmqGgdYd0GbkH+91aoGe7
# bkMWTsrRgszr9WSNe/9+qQlJM5pcdn3ldIKJTHf2yfU63pH9FDO3uR20Mc3FaJnw
# NrWMYPnkBcbCBYneg+e+Ptu0/QBM9h4lfvv3oYIXQDCCFzwGCisGAQQBgjcDAwEx
...
# BQAEIOEWKOWrkJpQ5gu5ncwxmgC4qChKG63a6tVILTIj8fChAhEA4IZMQkZ/KUUS
# pjhl4f8V4xgPMjAyMzExMTMxNTMzNThaoIITCTCCBsIwggSqoAMCAQICEAVEr/OU
...
# DQEJBTEPFw0yMzExMTMxNTMzNThaMCsGCyqGSIb3DQEJEAIMMRwwGjAYMBYEFGbw
# KzLCwskPgl3OqorJxk8ZnM9AMC8GCSqGSIb3DQEJBDEiBCC4Rdq0vKkA7MOCIdt7
# xrE6I79JJBq/ct6xJnj79Q7qFjA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDS9uRt
# 7XQizNHUQFdoQTZvgoraVZquMxavTRqa1Ax4KDANBgkqhkiG9w0BAQEFAASCAgAY
# UNiwBPSLD10NjvCrSkIgS6q3Lar/HjgdHHATRHCe4SH0XUqJiZtRsmspQoEe5e8W
# gRx8I4ikm0/WyM9MJ9pFSGkKB8lxt/bxXYXKjQcvGefFeVpUug0gA9sJalj8aHzl
# Vq+u5ILcgREwu/TrX8SXIFVzYWcRSy5fhhGNIFaL3Lcz1ykENgZa+xR/GWm3Dub+
# Zav0UrS28uce7RWdSG1F18u+UDmvlKJT22kQdpKf8umXDmEMTHNKnbxZ7maH+wOn
# xGsLzE7rrol5/1QBwzutjbuNAeOTZKnq4FQd0RWCo3cNXTDzLzjISOnBow2MNMnw
# eCH9M8LLLqpF2Jb8kikn/yc6vx7dzhoj6xH0/ecqvuJIzMgL2fO6ISsyEHt1tFLw
# 2LpJ8KVsfE8Ep4XKZhM0ZRqylI8MC67O7kVJX/o/QdHTykXYT+K9Q/+iJ8FmAu0p
# QsXoZnwlR23Wa3+zQgS/vG8+s73ZXMDyWpCbhovWYRTCz2j+B8YPdCq6H1JxRJSd
# HhddwiXoYYMHUSlbsw/6qWXfZpq3LsBJntM0p0Jx4Su9M4I48CPK2QIrHuOcUD14
# h/kGJV1Lp+uSngaJjVeGZB2u4ZUfn9RJQ/v6s2u/LKf/SUiiVzpiPeU6YObRpgtW
# VTTbejD9AiB8UJki2v0/u9hTKAiefOafScMsWjosTA==

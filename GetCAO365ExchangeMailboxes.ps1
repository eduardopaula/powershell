...
# Get the Office 365 templates that manage the Azure tenants.
$o365ExchangeTemplates = Get-CAO365Templates -Connection $connection

# Prompt the Azure tenant name
$tenantName = Read-Host -Prompt 'Specify the name of the Office 365 tenant'
    
# Search for specified template or exit script if not found.
$selectedTemplate = $o365ExchangeTemplates | Where-Object { $_.DisplayName -eq $tenantName }

if (!$selectedTemplate)
{
     Throw 'The Azure tenant is not audited by Change Auditor.'
}

$mailboxes = $null

#$auditedMailboxes = Get-CAO365ExchangeMailboxes -Connection $connection -Tenant $tenantName -First 5
$mailboxes = Get-CAO365ExchangeMailboxes -Connection $connection -Tenant $tenantName
...
{
$mailboxes | Out-GridView -Wait -Title 'CAO365ExchangeMailboxes'
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBJyBKR18+xKEv9
# rrU/3RHt/TSK1+DD7vR4y5ZxiEKhTqCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCD9KroXOgSAWxj3k0oo+/gibKf+LgYDsJ+D8WyKuy2j6jANBgkqhkiG
# 9w0BAQEFAASCAQB3WMbTQtaXCBjGyyqcInv0ENTqNLiozkEIW192uB4SnZMq8TFe
# qzYS1XNrHCMeptHbs6qmUhUhiZbOMPAzq2bTN2oQQmj2aCT3vdpi0uDp+6KS9P1N
# U9DtGN+pAAqG6y8bXjowtPhCbIReeRMsOk/IpCPrSfADMuuwIdPoceOIv2sakoPY
# eizZu+gDJVKV0WP6TSEFWvd6tOLATLCpkv3WIP3vTH1pmz95QFC/QTR87/KldPeZ
# VmAak2GZADTFjao/2GsFL1xS1Wc0qij1lBVsjN+ss5mI1gdZ34NLeZ4Or4Rhqfgn
# 8q2TuB4upgK4FgebBPJNpR1b9GCq+5Eq8YFDoYIXQDCCFzwGCisGAQQBgjcDAwEx
...
# BQAEIIn2SUXF7b/Y5+AQTKdlW/zXepSltJg+8MKLx5hgnSWiAhEAlyXSLa3Nxn0R
# r1oXsZzKERgPMjAyMzExMTMxNTM0MDhaoIITCTCCBsIwggSqoAMCAQICEAVEr/OU
...
# DQEJBTEPFw0yMzExMTMxNTM0MDhaMCsGCyqGSIb3DQEJEAIMMRwwGjAYMBYEFGbw
# KzLCwskPgl3OqorJxk8ZnM9AMC8GCSqGSIb3DQEJBDEiBCCx0NH7QeKh8wc5Zwpg
# HEgSujB+nLcEoJTR8FWyHKYaFjA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDS9uRt
# 7XQizNHUQFdoQTZvgoraVZquMxavTRqa1Ax4KDANBgkqhkiG9w0BAQEFAASCAgAh
# pvqVkYX+PIqAqRIKRiFOHFTIsbSwkopGcIckepqmlcqlP69E/7mUIAVU3oUC1nCe
# 2kdZvpow7fTzmK3KELZRXbt6ka01/geCztl6UPojtr9ZfZwu2nVtG8L2UbfE7kQZ
# 1GdQUvKlIX6+QV8kCBTKPKe5UgK0MSG2vuWdn3J7XuuyECE5vB9deUDi4nZKVuI2
# 41Im+QpDZDVs70KbR+kMSMaQhtD4CZeLWSh94N1M5onTq13plGrFt4xcD2yGXcoT
# IrtLzNhoyZfNp3xw1qj7j8adGNw0yMwhPTv1cFvjRQ3dH2hIQFPPWFSuCjUktCUg
# 5h1HDwA/asw4TiX80m/7UorvvBcTuvOAesskjs671KYeSQS2Eoa7DkthIiUfCnm1
# p86DSEJUNdoNrNdXq0PglHu6AgNVRRsQY8TeZm0UyTDHc3gzUDcMc2lOfvIw0azj
# DQ61VkwmLBA6qHwHhgsT/MjgXhn1vx4WCJ/ezGdKgPo4jZhTz9dCw3ORW8eAaLVN
# m2UnW1ba43beU0Vy9K7dhEZcXYRSr8GRB4f9fkj01ovHCcfjNhyy/EoeCm5bB+tx
# AJs5PPpsWNic1+wzf/2BtdaZk4ycnS89ZS5MpBHpK7+gIk8ysyljD7QurS1Xcwc8
# cjxq4Tw4YwZpZ0ClGmJ1EsI6Y51rJ3L6mzsn95X+Dg==

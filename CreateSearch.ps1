...
# Create a new search from an XML search definition
$existingSearchName = Read-Host 'Specify the name of a search to pull the XML search definition from'
$searchIsPublic = Read-Host 'Specify whether the search being created is public by entering "true" or "false"'
$searchPath = Read-Host 'Specify the desired path of the new search being created'
$newSearchName = Read-Host 'Specify the desired name of the new search being created'
if ($searchIsPublic.Equals("true"))
{
	Get-CASearches $connection | ? {$_.Name -eq $existingSearchName} | Get-CASearchDefinition $connection | Add-CASearch $connection -IsPublic -Path $searchPath -Name $newSearchName
}
else
{
	$userDomain = Read-Host 'Specify the domain of the target user'
	$userName = Read-Host 'Specify the user name of the target user'
	$objUser = New-Object System.Security.Principal.NTAccount($userDomain, $userName)
	$strSID = $objUser.Translate([System.Security.Principal.SecurityIdentifier])
	Get-CASearches $connection | ? {$_.Name -eq $existingSearchName} | Get-CASearchDefinition $connection | Add-CASearch $connection -UserSid $strSID.Value -Path $searchPath -Name $newSearchName
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD/vRq3nYXizRSo
# +p0BMKvSqaA1F4qzdaHYmt1irOPyWaCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCDTzJK00ArQR3JORD9UWhfsCjpoVd1lLosq0JylVPus/zANBgkqhkiG
# 9w0BAQEFAASCAQBWkk1wPnKsd/GjOszEtTLuBi49WrmAOARpg3U2r6OSJR5v5IZn
# zHEIRFr+40+W8FfVCH3Lcr9vvAbbP4Hq046qkj/4w4yNC+TEU8NvUGzjwzxv+kCP
# GuaR7mk+2MOu3BuBAFQggOPbgKRDH1LwCpDlAyCrM3mXmXYU4sGiuxgUE6TqPxTw
# uHhAzPacS4xK4HRElqDCbAL7oJefX+Cu9nV7YsKJh/39jD0OfTdnPFXlY4ay7Nq4
# T3Pq0A3ZO4fC+L83xlT0gu1ReRG4p5miiA9ExlO7fm2vkVfxDaiuAu9D8bst2V4k
# 5xTwathOnZa3u9Yf9qtAmWY4mmL93ygh5yRHoYIXPzCCFzsGCisGAQQBgjcDAwEx
...
# BQAEICJOmQ39/UXeyxZcXnGmCW+3RElofsziKIDTQQFi2jKAAhB0Hcp3SEE/Lm7L
# XO8uk1gtGA8yMDIzMTExMzE1MzQwMVqgghMJMIIGwjCCBKqgAwIBAgIQBUSv85Sd
...
# AQkFMQ8XDTIzMTExMzE1MzQwMVowKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQUZvAr
# MsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEIOlv1WR1HjXUNZyvNyqH
# 3HON83B5gXbqSZWUU/6XjZ1AMDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3t
# dCLM0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICACf5
# XCsh9ITTx5/K2u5Uq4rRmXrGUqSaLQM8/ypWPdzI3cJByLVAFudIbs+hXeuIxTEg
# bJ6BXDRZWeKZ7yYKXHPTt/jDM+nC1NBpbNedkyoq1YXAAbIHMv7dK0c00effj9Ti
# niWO7JNRdbxCpKnN7C6fLjj5jEpxvtl3w6oNBXGHdG7M1UXnzBnEhWWZQ3lFYKfO
# F50xZpD/yk0pytBYqCFICMuS+/5mDuJNnDJdHL/n6DHtZQhLXzQbgk496UCcbizj
# gAiXRXwIE5xb91xe4PCbDnm7l/cazBv2ddQVUwyOJt2mJPVci6MD3jNqkIF2FU5R
# wmORLI8TP74mfSxC/eMzIfFHu6eS0mO9X6uEGI01QQaWnTOacV8szM40WLi6zl8Q
# V9VH5GmpsAZyI+c3ZGIKqApModSGO/FCHvZ8J+XS13LgJZwYSZtAokI6CcAPVF+Z
# aM7Z2My4eYiQuIl/dVv0Nki6Psmqla5eF3cdFs7jE+5ELtPV6empwyHUzTBffiUE
# wIsr79SBaBx75jD+M3U1C2XZI9DFClBIZLA4wyDxBg22bwU+kEyDA9av+vOetPlj
# EZMiJSrDHGdVJd9RNOP/P3vbrBd3dOYqnWvt+NPJ6/Yi+jrWaVb6dGYTVckwRd5i
# xAjE8unTbH6De3hvYRoiqY4kwqNa6OjfQJ1doM+S

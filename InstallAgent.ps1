...
# Prompt for machine name and credentials
$machineToInstallAgentOn = Read-Host 'Target Machine (DNS Form)'

# Prompt for credentials (username/password) if this was an interactive script
$credsToUseForInstallation = Get-Credential -Message 'Enter credentials to use to install the agent on the target machine'

# When you have to provide credentials in non-interactive script, you can create a PSCredential directly
# $username = 'DOMAIN\User'
# $secureStringPassword = ConvertTo-SecureString "UserPasswordInPlainText" -AsPlainText -Force
# $credsToUseForInstallation = New-Object System.Management.Automation.PSCredential ($username, $secureStringPassword)

# We don't need to specify an install time, but if we want can schedule it for some time in the future
# When working with time in Change Auditor PowerShell, time usually must be specified in Universal Time.
# Example: -OperationTime $installTime
# $installMinutesFromNow = 2;
# $installTime = (Get-Date).ToUniversalTime().AddMinutes(2)

# Install an agent
Install-CAAgent -Credential $credsToUseForInstallation -MachineName $machineToInstallAgentOn -Connection $connection
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA/gplN6iL4+FuY
# PzVDcwtDEabZI3+/1X/kl2i9GEzDGKCCDB0wggXzMIIE26ADAgECAhAJvF2jX0kc
...
# DQEJBDEiBCB6bWPcy87J0obTBuYl8LcMCqlJEsVsYLZIIYFP/76T8jANBgkqhkiG
# 9w0BAQEFAASCAQAL8wW0OIo7fHXsoBEGeDEz9GDuUhnN+/pZU/YXRtW/DkYwdVOx
# R+8HoHdS+BoCpHVo+uMMOXepVTKyfAjyDHeAK9IgxTwW+Lv4CM+B5Ge4A1u/9eaL
# sK4mEuUD9vKQQ+voIFLfDAwJaropFs8OrliDX9z50VAO0e/B4uhlkv701nhI7C9M
# rlgaRv1fp2G+qMYK8zMwrPlkRcp83HjkZC3eywh2q8QWVTky1goILgQ+pooxJI4b
# 9NJjvBglgXHZ4jJ1wLRBy00v9oMdWmyU1fGNjBOnD0KSEjYrX7l6zllNezsuSnFG
# pCxGTZCOOumFo1qA+9CK8GWHreHlUVmW6LoMoYIXPzCCFzsGCisGAQQBgjcDAwEx
...
# BQAEICPyTkTfd1Keo6fiF5pxXYftMMRlmNUPusIP61j7WPIyAhAQPoifq104/6ri
# xdL3Xy3XGA8yMDIzMTExMzE1MzUzNVqgghMJMIIGwjCCBKqgAwIBAgIQBUSv85Sd
...
# AQkFMQ8XDTIzMTExMzE1MzUzNVowKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQUZvAr
# MsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEIBA8T4yw6/Mhoet5B07D
# LDl/c4fDMFeAbHgkLNokGRnvMDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3t
# dCLM0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICAHOe
# rC4SFu2JRaDtV8WaOsA4i8gz21HSVEZvSyheEtc79xDkHYnYrq3xoCwDcBLpx83M
# iWJ7FoA3nHBfwzbG4f2fjjHNcnB2Hm62wpKPv1GZPdt4ZNdzT6bGhrXwXktSGKS8
# t0h1Y7s8+IRowjGjfBz2+7ibwWXt7u1qUMuOAkImoMP8lBNnFO8Q1O1p60hOBRxr
# qVZBsItNKJiekvImIvY38wYgsOp6wLjjHR8r0/6tRfZiEhbG9I1VuxWdbYWgxBsu
# PIdtTRVUheQIJwBLa7DbTZeTIkbuVL0ZW+4dRoWJjrc3rRCKHOa2RLZsBY2amNAT
# XsbLuD8CTswj7IBNCpwLX2wAxUYLB+WbaMt5usjME/vnbTye5Y7iBWUG34Ajx8zX
# CfaXXwvNPXI/Fq0pClL0y0B7gsX782dH5g1HPKddcWx8H1Ep0KdnPgDgSlymtCFR
# rh32SrgG+1fcf/f2YIhmHe/GVYXDjm0qCVjxSoNlNvpgcGK5VLmfTIVOc8pK2NV8
# Jxkswdc+JYvc83l1ANlpxKefcy60BFwhCj00EDeSSmBD1GTSUN+opkbelRR9ZoKr
# KDp0P9eCb0FNjIjWtvnnco4hGbEtx9Qdu+fkAVV3oBlcUJ9drKwS2C115+vPEIlT
# oSWSBUxcvreqwz81EyCmKEJNo2iGsQuUuhOs1KkD
